...
export function parseSelectedExecutionTarget(value) {
  ...
  if (raw.startsWith("stack-")) {
    return {
      ...
      selectionType: "stack",
      workloadId: null,
      stackId: raw.slice("stack-".length) || null,
    };
  }
...
function normalizeTrackedResources(trackedResources = {}) {
...
function inferStackAction({
  ...
  if (selectedTarget?.selectionType === "stack" && selectedTarget?.stackId) return "update";
...
export async function resolveBlueprintExecutionContext({
  ...
  const normalizedTrackedResources = normalizeTrackedResources(workloadDefaults?.trackedResources);
  ...
  const resolvedExistingStacks = [
    ...
    ...(parsedTarget.selectionType === "stack" && parsedTarget.stackId ? [parsedTarget.stackId] : []),
  ...
  return {
    target: {
      ...
      scope:
        parsedTarget.selectionType === "workload"
          ? "workload"
          : parsedTarget.selectionType === "stack"
          ? "stack"
          : "environment",
      ...
      stackId: parsedTarget.stackId || explicitExistingStack,
