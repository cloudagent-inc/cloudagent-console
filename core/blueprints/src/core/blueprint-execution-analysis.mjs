...
const ENVIRONMENT_CHANGE_SHAPE_JSON_SCHEMA = {
  name: "environment_change_shape",
  schema: {
    type: "object",
    additionalProperties: false,
    required: ["isSupportedByConfiguredMethod", "resourceChangeType", "reason"],
    properties: {
      isSupportedByConfiguredMethod: { type: "boolean" },
      resourceChangeType: {
        type: "string",
        enum: ["create_new_resource", "update_existing_resource"],
      },
      reason: { type: "string" },
    },
  },
...
function heuristicReadOnlyFallback({ blueprint }) {
  ...
  return {
    isReadOnly,
    reason: isReadOnly
      ? "Blueprint appears focused on review, inspection, discovery, or reporting rather than implementation."
      : "Blueprint contains implementation-oriented tasks that imply making changes.",
...
function buildMutatingSafetyFallback({ blueprint, reason }) {
  ...
  return {
    ...
    reason:
      reason ||
      "Classifier output could not be trusted, so the blueprint is being treated as mutating for safety.",
  ...
  return {
    ...
    implementation: {
      ...
      reason: isMutating
        ...
        : "Blueprint appears read-only, so direct read-only CLI access is sufficient.",
    },
    rewriteDirectives: {
...
function normalizeReadOnlyResult(parsed, { blueprint }) {
  const fallback = buildMutatingSafetyFallback({
    blueprint,
    reason:
      "Classifier output could not be parsed reliably, so the blueprint is being treated as mutating for safety.",
  });
  ...
  if (!next.isReadOnly && !next.mutationTaskIds.length) {
    const safetyFallback = buildMutatingSafetyFallback({
      blueprint,
      reason:
        "No mutating task ids were identified from classifier output, so the blueprint is being treated as mutating for safety.",
    });
  ...
  if (!next.isMutating) {
    next.implementation = {
      ...
      reason:
        ...
        "Blueprint is read-only, so deployment method and stack action are not needed.",
    };
...
function normalizeTargetRecommendationResult(parsed, { candidates = [] } = {}) {
  ...
  const fallback = {
    ...
    reason:
      "No workload was strongly identified for this blueprint, so environment/account scope is recommended.",
    topCandidate: null,
    candidates: [],
  };
...
function normalizeEnvironmentChangeShapeResult(parsed, { configuredMethod = null } = {}) {
  const fallback = {
    isSupportedByConfiguredMethod: configuredMethod === "cloudformation" || configuredMethod === "terraform" || configuredMethod === "opentofu",
    resourceChangeType: "create_new_resource",
    reason:
      "No reliable environment-wide change-shape assessment was available, so the fallback assumes a create-new path with the configured method if that method is IaC-capable.",
  };
  const next = parsed && typeof parsed === "object" ? { ...parsed } : { ...fallback };
  if (typeof next.isSupportedByConfiguredMethod !== "boolean") {
    next.isSupportedByConfiguredMethod = fallback.isSupportedByConfiguredMethod;
  }
  if (!["create_new_resource", "update_existing_resource"].includes(next.resourceChangeType)) {
    next.resourceChangeType = fallback.resourceChangeType;
  }
  if (typeof next.reason !== "string" || !next.reason.trim()) {
    if (next.isSupportedByConfiguredMethod) {
      next.reason =
        next.resourceChangeType === "update_existing_resource"
          ? `The configured ${configuredMethod || "infrastructure-as-code"} method can support this change when the affected resource is managed by that method, and the blueprint appears to update existing resources.`
          : `The configured ${configuredMethod || "infrastructure-as-code"} method can support this change, and the blueprint appears to create new resources.`;
    } else {
      next.reason =
        next.resourceChangeType === "update_existing_resource"
          ? `The blueprint appears to update existing resources, but the configured ${configuredMethod || "deployment"} method is not a reliable implementation path for this kind of change.`
          : `The configured ${configuredMethod || "deployment"} method is not a reliable implementation path for this kind of change.`;
    }
  }
  return next;
}
...
export async function classifyBlueprintReadOnly({ blueprint } = {}) {
  ...
  if (!OPENAI_TOKEN) {
    return buildMutatingSafetyFallback({
      blueprint,
      reason:
        "The classifier model was unavailable, so the blueprint is being treated as mutating for safety.",
    });
  }

  const instructions = `
You are classifying whether a blueprint that describes tasks to be executed in an AWS environment is read-only or includes tasks that update the environment (create/update/delete/enable/disable/etc.)
...
Rule:
- If all tasks are review, discovery, audit, inspection, validation, inventory, or reporting only, the blueprint is read-only.
- If at least one task requires implementing changes or updates in the environment, the blueprint is not read-only.
- If the blueprint is read-only, mutationTaskIds must be an empty array and the execution method should be treated as AWS CLI only because no deployment method is needed.
- If the blueprint is not read-only, return the exact task ids that require creating, updating, deleting, enabling, disabling, or otherwise changing the environment in mutationTaskIds.
- If the blueprint is not read-only, mutationTaskIds must contain at least one task id from the provided plan.
- If you are uncertain, prefer classifying the blueprint as not read-only and include the most likely mutating task ids.
...
export async function analyzeBlueprintExecution({
  ...
  const instructions = `
You are analyzing a blueprint execution context after mutation classification.
...
Constraints:
- Do not re-decide whether the blueprint is read-only.
...
- If readOnlyResult.isReadOnly is true, do not return cloudformation, terraform, or opentofu because read-only blueprints do not require a deployment mechanism.
...
- If the blueprint appears environment/account scoped, set workloadScope.status to does_not_fit and suggestSeparateDeployment=true.
`;
...
export async function recommendBlueprintExecutionTargets({
  ...
  const instructions = `
You are deciding whether a blueprint should run at environment/account scope or against one of the provided workloads.

Return strict JSON only.

Decision rules:
- Recommend a workload only when the blueprint clearly fits that workload's purpose or tracked resources.
- Recommend environment/account scope when the blueprint is shared, account-wide, platform-wide, or does not clearly map to a single workload.
...
export async function assessEnvironmentChangeShape({
  blueprint = null,
  ...
  const normalizedMethod = ["cloudformation", "terraform", "opentofu"].includes(configuredMethod)
    ? configuredMethod
    : null;
  const fallback = normalizeEnvironmentChangeShapeResult(null, {
    configuredMethod: normalizedMethod,
  });

  if (!normalizedMethod || !analysis?.isMutating) {
    return fallback;
  }
  ...
  const payload = {
    blueprint: buildMutatingTaskAssessmentPayload(
      blueprint,
      Array.isArray(readOnlyResult?.mutationTaskIds) ? readOnlyResult.mutationTaskIds : []
    ),
    configuredMethod: normalizedMethod,
    readOnlyResult: readOnlyResult
      ? {
          isReadOnly: Boolean(readOnlyResult.isReadOnly),
          reason: readOnlyResult.reason || "",
          mutationTaskIds: Array.isArray(readOnlyResult.mutationTaskIds)
            ? readOnlyResult.mutationTaskIds
            : [],
        }
      : null,
    executionContext: {
  ...
  const instructions = `
You are assessing an environment-wide mutating blueprint against one fixed implementation method.

Return strict JSON only.

You must answer two questions:
1. Does the configured method support making this change?
2. Is this blueprint creating a new resource or updating an existing resource?

Inputs:
- configuredMethod is fixed and must not be changed by you.
- The method is one of: cloudformation, terraform, opentofu.
- blueprint.title and blueprint.description describe the overall blueprint.
- blueprint.mutatingTasks contains only the tasks that were already identified as mutating by a previous classifier.
- Focus primarily on blueprint.mutatingTasks when deciding support and change shape.
- Do not infer support or change shape from non-mutating tasks that are not included here.

Tooling:
- You have access to web_search.
- Use web_search when needed to verify whether the configured method supports the mutating task changes.
- Prefer official documentation and primary sources when checking CloudFormation resource support or Terraform/OpenTofu provider/resource support.
- Do not use web_search to choose a different method. Use it only to verify whether the fixed configured method supports the change.

Decision rules:
- isSupportedByConfiguredMethod means whether the configured method can represent and perform this kind of change in principle.
- Do not treat "no specific managing stack/repo has been identified yet" as unsupported.
- Do not treat "the resource already exists" as unsupported by itself.
- If the configured method can create the required new resources, isSupportedByConfiguredMethod should be true.
- If the configured method can update this kind of resource when that resource is managed by the method, isSupportedByConfiguredMethod should be true.
- Set isSupportedByConfiguredMethod to false only when the configured method itself is not a realistic implementation path for this type of change.
- resourceChangeType should be "create_new_resource" when the blueprint is primarily creating brand-new resources.
- resourceChangeType should be "update_existing_resource" when the blueprint is primarily modifying configuration or behavior of resources that are assumed to already exist.
- Example: creating an EBS volume is create_new_resource.
- Example: enabling logging for an existing S3 bucket is update_existing_resource.
- Example: creating an AWS budget with notifications is create_new_resource and should usually be supported by CloudFormation if CloudFormation supports those resource types.
- Example: enabling or modifying settings on an already-existing bucket is update_existing_resource even if you do not yet know which stack or repo manages the bucket.
- Do not choose direct cloud CLI here. Only assess support for the configured method and the change shape.
- If uncertain whether a change is create-new or update-existing for an already-present resource, prefer update_existing_resource.
- If uncertain about support for a create-new infrastructure blueprint, prefer isSupportedByConfiguredMethod=true.
`;
  ...
  try {
    const tools = [
      webSearchTool({
        name: "web_search",
        description:
          "Search the web for official CloudFormation, Terraform, OpenTofu, and cloud provider support details.",
      }),
    ];
    const agent = new Agent({
      name: "EnvironmentChangeShapeAgent",
      instructions,
      model,
      tools,
      responseFormat: { type: "json_schema", json_schema: ENVIRONMENT_CHANGE_SHAPE_JSON_SCHEMA },
    });
    const result = await run(agent, [user(JSON.stringify(payload))], {
      maxTurns: 1,
      runConfig: { tracingDisabled: true, reasoning: { effort: "medium" } },
    ...
    try {
      console.log(
        "[BLUEPRINT_ENVIRONMENT_CHANGE_SHAPE_LLM_RESULT]",
        JSON.stringify(result, null, 2)
      );
    } catch (error) {
      console.log(
        "[BLUEPRINT_ENVIRONMENT_CHANGE_SHAPE_LLM_RESULT_LOG_ERROR]",
        error?.message || String(error)
      );
    }
    const raw = extractModelTextOutput(result);
    const parsed = extractJsonStrict(raw);
    const normalized = normalizeEnvironmentChangeShapeResult(parsed, {
      configuredMethod: normalizedMethod,
    });
    try {
      console.log(
        "[BLUEPRINT_ENVIRONMENT_CHANGE_SHAPE_OUTPUT]",
        JSON.stringify(
          {
            configuredMethod: normalizedMethod,
            raw,
      ...
      console.log(
        "[BLUEPRINT_ENVIRONMENT_CHANGE_SHAPE_OUTPUT_LOG_ERROR]",
        error?.message || String(error)
      );
    }
    return normalized;
...
export async function determineBlueprintUpdateStrategy({
  ...
  const fallback =
    !analysis?.isMutating
      ? {
          ...
          targetType: "none",
          selectedStackId: null,
          reason: "Read-only blueprints do not require an update strategy.",
          planSummary: "No create/update decision is required for a read-only blueprint.",
        }
  ...
  const instructions = `
You are deciding how a mutating blueprint should be applied when the current implementation method is CloudFormation, Terraform/OpenTofu, or direct cloud CLI.

Return strict JSON only.

Decision logic:
- If the resources affected by the blueprint are already managed by an existing template/repository target, prefer update_existing.
- If the resources are not managed by an existing template/repository target and the blueprint is creating new infrastructure, prefer create_new and keep the currently resolved template method.
- If the resources are not managed by an existing template/repository target and the blueprint is modifying existing infrastructure that is assumed to already exist, prefer method="aws_cli" with changeStrategy="direct_aws_cli". For Azure targets, this means Azure CLI even though the legacy method value is aws_cli.
...
- If the environment/workload method is cloudformation, terraform, or opentofu and the blueprint is simply creating new resources, keep that method.
