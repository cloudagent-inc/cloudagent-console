// Helpers for evaluating and preparing blueprint execution strategies
// - rewriteBlueprintForExecution: adjust blueprint tasks to honor execution preferences and defaults
...
} from "./blueprint-execution-context.mjs";
...
export async function rewriteBlueprintForExecution({
  ...
  const instructions = `
You are rewriting a blueprint to align it with execution preferences, resolved target context, and the chosen configuration method. The agent has these tools:
...
3) If configuration_mode is cloudformation, consolidate configuration tasks that can be handled by one template into as few tasks as practical (ideally one). Update ids/titles/descriptions accordingly. Keep separate tasks only when CLI is required or separation is necessary. When consolidating, adjust completionCriteria and executionPlan to reflect the CFN path and reference existing_stack or existing_stacks when provided.
   4) Use execution_context.workload.trackedResources when the selected workload is directly relevant to the blueprint. General rule: if the blueprint operates on a resource type or service that is already represented by the selected workload's tracked resources, scope the blueprint to only those tracked resources. Replace broad environment-wide discovery, listing, filtering, validation, and mutation steps with workload-scoped steps that reference only the tracked resource ids, arns, names, stack ids, or repo paths supplied in execution_context. If the context suggests the blueprint is environment/account scope instead, do not force workload-tracked scoping into unrelated tasks.
   ...
   10) If execution_context.workload.selected is true and the blueprint is mutating, add a final task at the end to reconcile workload metadata only when needed:
      ...
      - For read-only blueprints, never add a workload update task.
...
Important:
- Always return valid JSON matching the schema: { "blueprint": <rewritten blueprint>, "meta": { ... } }
