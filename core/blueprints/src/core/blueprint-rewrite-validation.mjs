...
export function validateRewrittenBlueprint({
  ...
  if (!Array.isArray(phases) || phases.length === 0) {
    return {
      ok: false,
      errors: ["Rewritten blueprint does not contain a valid plan array."],
      warnings,
    };
  }
  ...
  if (analysis?.isMutating && !executionContext?.deployment?.resolvedMethod) {
    errors.push("Mutating blueprint rewrite is missing a resolved deployment method.");
  }
  ...
  if (
    ...
    errors.push("Repo-driven delivery was resolved but the rewritten blueprint does not mention repo or pipeline flow.");
  }
  ...
  if (
    ...
    warnings.push("Tracked resource scoping was expected but is not clearly reflected in the rewritten blueprint.");
  }
