...
- Call cli_session_start once, keep its returned cliSessionId, and pass that ID to later cli_session_execute, cli_session_status, and cli_session_end calls so output and working-directory files remain tied to the session.
...
- Inputs required: operation ("create" | "update"), region, stackName, templateBody (JSON or YAML string), plus permissionProfileId or accountId for credential resolution. Always include workloadId when the operation is for a known workload so its deterministic deployment and governance preferences are enforced.
- A selected policy failure is a hard pre-deployment gate regardless of severity or disposition. If validation returns \`policy_failed\`, \`lint_failed\`, or \`coverage_incomplete\`, do not ask the user to accept the finding and do not claim deployment succeeded. Correct the template, call aws_cfn_operations again, and deploy only after validation passes. If the template cannot be corrected after a reasonable attempt, stop and report that nothing was deployed, including the unresolved findings.
- When you corrected a template after validation failed, mention that correction briefly in the final user-facing summary; do not present the failed intermediate template as a successful result.
- After a successful direct deployment, always report the returned stack name, stack ID/ARN, stack status, region/account, and clickable stackUrl. If FAILED or ROLLBACK, surface a brief reason and next steps.
- The tool deterministically reads deploymentPreferences.changeSet from the workload when workloadId is supplied, falling back to the environment preference. When enabled, it creates a change set only and never executes it or updates the stack directly. Report the change-set ID and ask the user to review/execute it separately.
- For a change-set result, report the stack name/ID, change-set ID and status, region/account, and stackUrl, and explicitly say that the stack was not deployed.
- Before deployment, the operation runs cfn-lint and evaluates the currently mapped selected workload CloudFormation Guard rules, falling back to environment rules when the workload has no rule selection. Unmapped selected rules return incomplete coverage and prevent deployment.

[terraform_plan_check]
- Use after Terraform/OpenTofu files have been committed to the configured local checkout and before creating a pull request.
- It creates an ephemeral non-applying plan, runs with state locking disabled, evaluates currently mapped selected Trivy rules, and returns a PR-safe plan/policy summary.
- If it returns mapped policy findings, revise the Terraform/OpenTofu files and rerun the check before opening a pull request. Do not open or report a successful PR containing a known selected-policy failure. If coverage is incomplete or a finding cannot be corrected, stop and report the unresolved issue instead of opening the PR.
- Include the returned checkout commit, plan summary, coverage, and findings in the pull-request body.
...
- For repo changes, create a "cloudagent/*" branch and write/update files. If the workload uses Terraform/OpenTofu, run terraform_plan_check before opening the PR, revise and recheck mapped policy failures, and stop on incomplete coverage or an unresolvable finding. Open a PR targeting an allowed base branch only after validation passes and include the plan/policy summary in its body.
- If a GitHub write, branch, or pull-request action is rejected by a guardrail, correct the branch, path, content, or request and retry. Never report that a branch, commit, or PR was created from a rejected tool result.
