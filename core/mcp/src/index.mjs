...
import {
  DeploymentPreferencesSchema,
  SecurityRulesSchema,
  TrackedResourcesSchema,
} from "@cloudagent/cloudagent-tools/tools/tool_workload_update";
...
function createLocalMcpServer({
  ...
  function buildTools(sessionId, req) {
    ...
    return createCloudAgentTools({ store }).tools;
  }
  ...
  function buildLocalToolRunContext(req, { requestId, toolName } = {}) {
    const runContext = getRunContext(req);
    return {
      userId: LOCAL_MCP_USER_ID,
      recordId: runContext.recordId,
      cloudagentRunId: runContext.recordId,
      cliSessionScopeId: runContext.recordId,
      runner: runContext.runner,
      permissionProfileId: runContext.permissionProfileId,
      accountId: runContext.accountId,
      region: runContext.region,
      recordContextEvent: (event = {}) => {
        emitLocalToolEvent(req, {
          requestId,
          toolName: event.sourceTool || toolName,
          ...event,
        });
      },
    };
  ...
  async function invokeLocalToolDirect(toolName, args, req) {
    ...
    try {
      const result = await localTool.invoke(
        { context: buildLocalToolRunContext(req, { requestId, toolName }) },
  ...
  function createServer() {
    ...
    registerLocalTool(
      "update_workload",
      {
        ...
        inputSchema: {
          ...
          deploymentPreferences: DeploymentPreferencesSchema.nullable().optional(),
          securityRules: SecurityRulesSchema.nullable().optional(),
          trackedResources: TrackedResourcesSchema.nullable().optional(),
    ...
    mcpServer.registerTool(
      "cli_session_start",
      {
        ...
        description: "Start or reuse a local CloudAgent CLI session for the current run scope and selected environment. Keep the returned cliSessionId for later commands; set forceNew only when a distinct session is required.",
        inputSchema: {
          accountId: z.string().nullable().optional(),
          permissionProfileId: z.string().nullable().optional(),
    ...
    mcpServer.registerTool(
      "aws_cfn_operations",
      {
        ...
        description: "Validate a CloudFormation template with cfn-lint and selected mapped CloudFormation Guard rules, then create or update the stack using selected local CloudAgent environment credentials. Always pass workloadId for a known workload. If its effective deploymentPreferences.changeSet setting is enabled, the tool creates a change set only and never executes it or updates the stack directly.",
        inputSchema: {
    ...
    mcpServer.registerTool(
      ...
      {
        title: "Terraform Plan Check",
        description: "Create an ephemeral Terraform/OpenTofu plan with workload environment credentials, evaluate selected Trivy guardrails, and return a PR-safe summary. Never applies changes and disables state locking.",
        inputSchema: {
          workloadId: z.string(),
      ...
      (args, req) => invokeLocalTool("terraform_plan_check", args, req)
    );
...
export function createLocalMcpRouter({
  ...
  if (typeof createCloudAgentTools !== "function") {
    throw new Error("createLocalMcpRouter requires createCloudAgentTools");
  }
