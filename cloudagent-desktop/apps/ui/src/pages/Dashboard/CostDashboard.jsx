...
const summarizeCostPayloadDebug = (payload = {}) => {
  const spend = payload?.data?.spend && typeof payload.data.spend === 'object'
    ? payload.data.spend
    : {};
  const dailyTotal = Array.isArray(spend.dailyTotal) ? spend.dailyTotal : [];
  const monthlyTotal12m = Array.isArray(spend.monthlyTotal12m) ? spend.monthlyTotal12m : [];
  const dailyByService = Array.isArray(spend.dailyByService) ? spend.dailyByService : [];
  const dailyByLinkedAccount = Array.isArray(spend.dailyByLinkedAccount) ? spend.dailyByLinkedAccount : [];
  const rightsizingRecommendations = Array.isArray(payload?.data?.rightsizing?.recommendations)
    ? payload.data.rightsizing.recommendations
    : [];
  const savingsPlansRecommendations = getSavingsPlansRecommendationItems(payload?.data?.savingsPlans?.recommendationsByType);
  const costOptimizationHubRecommendations = Array.isArray(payload?.data?.costOptimizationHub?.recommendations)
    ? payload.data.costOptimizationHub.recommendations
    : [];

  return {
    generatedAt: payload?.generatedAt || payload?.analysis?.cost?.generatedAt || '',
    cacheSource: payload?.output?.cache?.source || (payload?.output?.cache?.cacheHit ? 'cache' : ''),
    artifactPath: payload?.analysis?.cost?.path || payload?.output?.artifact?.path || payload?.output?.artifact?.objectKey || '',
    checkCount: countArray(payload?.checks),
    dailyTotalRows: dailyTotal.length,
    monthlyTotalRows: monthlyTotal12m.length,
    serviceRows: dailyByService.length,
    linkedAccountRows: dailyByLinkedAccount.length,
    anomalyCount: countArray(payload?.data?.anomalyDetection?.anomalies),
    recommendationCount:
      rightsizingRecommendations.length +
      savingsPlansRecommendations.length +
      costOptimizationHubRecommendations.length,
    spendTotal: dailyTotal.reduce((sum, entry) => sum + (toNumber(entry?.amount ?? entry?.total ?? entry?.cost) || 0), 0),
    firstDailyDate: dailyTotal[0]?.date || '',
    lastDailyDate: dailyTotal[dailyTotal.length - 1]?.date || '',
    hasSpendRows: dailyTotal.length > 0 || dailyByService.length > 0 || dailyByLinkedAccount.length > 0,
  };
};
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-hidden bg-white">
        ...
        {diagnostics.length > 0 && (
          <div className="mt-4 overflow-y-auto max-h-[34vh] border rounded-lg">
            <Table>
              <TableHeader>
                <TableRow className="bg-gray-50">
                  ...
                  <TableHead className="text-right">Linked Accts</TableHead>
              ...
              <TableBody>
                {diagnostics.map((item) => (
                  <TableRow key={item.permissionProfileId} className="hover:bg-gray-50">
                    <TableCell>
                      <div className="font-medium text-gray-900">{item.name || item.permissionProfileId}</div>
                      <div className="text-xs text-gray-500">{item.permissionProfileId}</div>
                    </TableCell>
                    <TableCell>
                      <div className="text-sm text-gray-900">{item.requestStatus || 'unknown'}</div>
                      {item.requestError && (
                        <div className="max-w-48 truncate text-xs text-red-600" title={item.requestError}>
                          {item.requestError}
                        </div>
                      )}
                    </TableCell>
                    <TableCell className="text-right">{item.dailyTotalRows}</TableCell>
                    <TableCell className="text-right">{item.serviceRows}</TableCell>
                    <TableCell className="text-right">{item.linkedAccountRows}</TableCell>
                    <TableCell className="text-right">{item.checkCount}</TableCell>
                    <TableCell>
                      <div className={item.hasPayload ? 'text-xs text-emerald-700' : 'text-xs text-amber-700'}>
                        {item.hasPayload ? 'Loaded' : 'Missing'}
                        {item.hasPayload && !item.hasSpendRows ? ' - no spend rows' : ''}
                      </div>
                      <div className="max-w-56 truncate text-xs text-gray-500" title={item.artifactPath || item.cacheSource || ''}>
                        {item.artifactPath || item.cacheSource || 'No artifact metadata'}
                      </div>
                      {item.generatedAt && (
                        <div className={`text-xs ${getFreshnessTextClass(item.generatedAt)}`}>
                          {formatRelativeTime(item.generatedAt)}
                          {getFreshnessSuffix(item.generatedAt)}
                        </div>
                      )}
                    </TableCell>
                  </TableRow>
...
export default function CostDashboard() {
  ...
  const resolveEnvironmentCostReport = useCallback(
    async (permissionProfileId, forceRefresh = false) => {
      ...
      if (!forceRefresh && !existing && !isLocalMode && !isAzureProfile(profile) && !hasStoredCostMetadata(profile)) {
        return null;
      }
    ...
    [dispatch, environmentCostResultsById, isLocalMode, scopedProfilesById]
  );
  ...
  const displayEnvironmentSummaries = usesAggregateStyleLayout
    ...
    : [];
  const costLoadDiagnostics = useMemo(() => {
    const diagnosticSources = isAggregateScope
      ? aggregateSourceEnvironments
      : isAzureTenantScope
        ? selectedAzureTenantSourceEnvironments
        : selectedEnvironment
          ? [{
              ...selectedEnvironment,
              permissionProfileId: selectedEnvironmentSourceId,
            }]
          : [];

    return diagnosticSources
      .map((environment) => {
        const permissionProfileId = String(
          environment?.sourcePermissionProfileId || environment?.permissionProfileId || ''
        ).trim();
        if (!permissionProfileId) return null;
        const record = environmentCostResultRecords?.[permissionProfileId] || null;
        const payload = record?.payload || environmentCostResultsById?.[permissionProfileId] || null;
        const request = environmentCostRequestRecords?.[permissionProfileId] || null;
        const payloadSummary = summarizeCostPayloadDebug(payload || {});
        return {
          permissionProfileId,
          name: environment?.name || environment?.sourceProfileName || permissionProfileId,
          type: environment?.type || '',
          accountId: environment?.accountId || environment?.subscriptionId || environment?.tenantId || '',
          requestStatus: request?.status || (payload ? 'succeeded' : 'not loaded'),
  ...
  useEffect(() => {
    if (costLoadDiagnostics.length === 0) return;
    const diagnosticsSnapshot = {
      scope,
      isLocalMode,
      loading,
      hasReport: Boolean(report),
      diagnostics: costLoadDiagnostics,
    };
    if (typeof window !== 'undefined') {
      window.__cloudAgentCostDiagnostics = diagnosticsSnapshot;
    }
    const loadedWithoutSpendRows = costLoadDiagnostics.filter((item) => item.hasPayload && !item.hasSpendRows);
    if (loadedWithoutSpendRows.length > 0) {
      console.warn('[CloudAgent Cost] Artifacts loaded but no spend rows were found', diagnosticsSnapshot);
      return;
    }
    console.info('[CloudAgent Cost] Cost load diagnostics', diagnosticsSnapshot);
  }, [costLoadDiagnostics, isLocalMode, loading, report, scope]);
