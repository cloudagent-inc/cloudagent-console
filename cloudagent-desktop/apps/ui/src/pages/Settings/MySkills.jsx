...
} from '../../lib/skillLibraryNavigation';
...
const MySkills = () => {
  ...
  const [shouldDefaultEmptySkillsToLibrary, setShouldDefaultEmptySkillsToLibrary] =
    useState(() => getDashboardSkillTab(location.pathname) === 'skills');
  ...
  const loadCustomSkills = useCallback(async () => {
    const action = await dispatch(fetchSkills({ count: 50 }));
    if (fetchSkills.fulfilled.match(action)) {
  ...
  useEffect(() => {
    if (!shouldDefaultEmptySkillsToLibrary || !customSkillsLoaded || loading) return;

    setShouldDefaultEmptySkillsToLibrary(false);
    if (shouldDefaultToBrowseLibrary({
      requestedDefault: shouldDefaultEmptySkillsToLibrary,
      customSkillsLoaded,
      loading,
      error,
      userSkills,
    })) {
      navigate('/dashboard/skills/library', { replace: true });
    }
  }, [
    customSkillsLoaded,
    error,
    loading,
    navigate,
    shouldDefaultEmptySkillsToLibrary,
    userSkills,
  ]);
