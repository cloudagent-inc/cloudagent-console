...
const buildCodexSkillMarkdown = (blueprint = {}) => {
  ...
  const lines = [
    ...
    '## Instructions',
    '',
    '- Read `session-context.json` before acting. It contains the selected environment, workload, regions, preferences, local scan/report context, and the execution plan.',
    '- Use `session-context.json.environment.authProfile` to understand the selected AWS account/profile and region.',
    '- For cloud CLI work, use the CloudAgent MCP tools `cli_session_start` and `cli_session_execute`. Pass `permissionProfileId`, `accountId`, and `region` from `session-context.json.environment.authProfile`, and pass concrete commands such as `aws sts get-caller-identity --output json`.',
