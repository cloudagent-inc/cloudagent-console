...
const CLOUD_PROVIDER_OPTIONS = [
  { value: 'aws', label: 'AWS', description: 'Build a blueprint for AWS accounts and regions.' },
  { value: 'azure', label: 'Azure', description: 'Build a blueprint for Azure subscriptions and resource groups.' },
];
...
export default function BlueprintBuilder({
  ...
  function codexSkillFileName(title = 'cloudagent-blueprint') {
  ...
  function buildCodexSkillMarkdown() {
    const title = getPlanTitle() || skeleton?.title || planData?.title || 'CloudAgent Codex Blueprint';
    const description = getPlanDescription() || planDescription || planData?.description || '';
    const lines = [
      `# ${title}`,
      '',
    ...
    if (description) {
      lines.push('', '## Description', '', Array.isArray(description) ? description.join('\n') : String(description));
    }

    lines.push('', '## Cloud Provider', '', cloudProvider === 'azure' ? 'Azure' : 'AWS');
    ...
    if (defaultValues && String(defaultValues).trim()) {
      lines.push('', '## Plan Settings', '', String(defaultValues).trim());
    }

    lines.push('', '## Blueprint Plan', '', blueprintToMarkdown().trim());

    return `${lines.filter((line) => line !== null && line !== undefined).join('\n').replace(/\n{3,}/g, '\n\n')}\n`;
  }

  function handleDownloadCodexSkill() {
    try {
      ...
      link.href = url;
      link.download = codexSkillFileName(getPlanTitle() || skeleton?.title || planData?.title);
      link.click();
  ...
  function applyMarkdownDraft() {
    try {
      ...
      setMarkdownError(error?.message || 'Could not parse the markdown blueprint.');
    }
  }
  ...
  function buildLoadedPlanState(loadedPlan) {
    ...
    const parsedCredits = Number(loadedPlan?.credits);
    if (Number.isFinite(parsedCredits)) {
      nextState.credits = parsedCredits;
    }
    return nextState;
  }
  ...
  function buildClientPlanState() {
    ...
    const parsedCredits = Number(skeleton?.credits);
    const state = {
      planId,
      cloudProvider,
  ...
  async function handleSaveEdits() {
    try {
      ...
      try { toast.success('Blueprint updated'); } catch {}
      if (navigate) navigate('/dashboard/blueprints');
    } catch (e) {
      console.error('Save Blueprint error:', e);
      setApiError(e?.message || 'Failed to save blueprint');
      try { toast.error(e?.message || 'Failed to save blueprint'); } catch {}
    }
  }
  ...
  async function handleCompleteBlueprint() {
    try {
      ...
      if (navigate) navigate('/dashboard/blueprints');
      try {
        toast.success(isCloneMode ? 'Blueprint save started' : 'Blueprint generation started');
      } catch {}
    } catch (e) {
      console.error('Complete Blueprint error:', e);
      setApiError(e?.message || 'Failed to start blueprint generation');
      try { toast.error(e?.message || 'Failed to start blueprint generation'); } catch {}
    }
  }
  ...
  async function handleSaveAsNew() {
    try {
      ...
      if (navigate) navigate('/dashboard/blueprints');
      try { toast.success('Blueprint saved'); } catch {}
    } catch (e) {
  ...
  const showPersistentReviewChat = step === 2 && !!skeleton;
  const showSidebarChat = showPersistentReviewChat || (chatOpen && step >= 2);
  const chatPanelElement = React.useMemo(() => (
    <PlanBuilderChatPanel
      ...
      canClose={!showPersistentReviewChat}
    ...
    />
  ), [chatOpen, chatInput, step, chatLoading, autoGenerate, chatMessages, agentWarnings, showPersistentReviewChat, cloudProvider]);

  const showSettingsReview =
    step === 4 &&
    !isEditMode &&
    (permissionsPolicy || (defaultValues && defaultValues.trim().length));
  const currentBlueprintTitle = getPlanTitle() || skeleton?.title || 'Untitled Blueprint';
          ...
          <Card className="w-full max-w-6xl mx-auto border-0 shadow-xl bg-white/80 backdrop-blur-sm">
            <CardHeader className="pb-6">
              ...
              <p className="text-sm text-gray-600">
                {setupStep === 'planType'
                  ? 'Start by choosing whether this blueprint should review an environment or make configuration changes.'
                  : 'Tell the assistant what you want the blueprint to accomplish.'}
              </p>
            </CardHeader>
                        ...
                        <div className="space-y-4 pt-1">
                          ...
                          <div className="space-y-2">
                            ...
                            <Textarea
                              ...
                              rows={3}
                              placeholder="Optional constraints, scope, or style preferences for the generated blueprint."
              ...
              <div className="mt-4 flex items-center justify-between gap-4 flex-wrap">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {!isEditMode && !chatLoading && (
                    ...
                    >
                      {isCloneMode ? 'Save as New' : 'Complete Blueprint'}
                    </Button>
                  )}
                </div>
              </div>
              ...
              <div className="mt-4 p-3 rounded-lg bg-blue-50 border border-blue-100">
                <p className="text-sm text-blue-800">
                  {hasCompletedTaskDetails ? (
                    <>
                      Review and update the completed blueprint below. You can edit task titles, descriptions, execution plans, and completion criteria directly, or use the chat to request changes.
                    </>
                  ) : (
                    <>
                      Review the proposed steps below. You can edit task titles and descriptions directly, or use the chat to request changes. When you're satisfied, click <strong>Complete Blueprint</strong> to generate the full execution details.
                    </>
                  )}
                </p>
              </div>
