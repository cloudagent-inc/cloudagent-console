...
export default function CodexBlueprintSkillEditor({ planData }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError('');
      try {
        const response = await codexClient.getBlueprintSkill(recordId);
        if (!mounted) return;
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const response = await codexClient.updateBlueprintSkillFile(recordId, {
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <h1 className="text-2xl font-semibold text-gray-900">
              {planData?.title || 'Codex Blueprint'}
            </h1>
            <p className="mt-1 text-sm text-gray-600">
              Edit the Codex skill files used when this blueprint is handed off to Codex CLI.
            </p>
          </div>
        </div>
