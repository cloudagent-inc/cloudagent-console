import { extractGithubGuardrailRejection } from '@/lib/githubGuardrails';
...
export function normalizeGithubOperation({ output, input = null }) {
  ...
  // Structured GitHub guardrail rejection (protected_branch, secret_detected, ...).
  // Never carries a matched secret value — only the pattern name.
  const guardrail = extractGithubGuardrailRejection(toolOutput);
