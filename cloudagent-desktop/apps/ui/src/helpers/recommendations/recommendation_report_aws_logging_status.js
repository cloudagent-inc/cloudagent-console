...
const RULE_ID = 'logging_status_report';
const LOGGING_REPORT_ID = 'report_aws_backup';
...
const createReason = (missingProfiles) => {
  if (!missingProfiles.length) {
    return 'No Logging Status report has run in the past 30 days.';
  }
  ...
  if (!accountList.length) {
    return 'No Logging Status report has run in the past 30 days for your connected environments.';
  }

  if (accountList.length === 1) {
    return `No Logging Status report has run in the past 30 days for account ${accountList[0]}.`;
  }

  if (accountList.length === 2) {
    return `No Logging Status report has run in the past 30 days for accounts ${accountList[0]} and ${accountList[1]}.`;
  }

  const allButLast = accountList.slice(0, -1).join(', ');
  const last = accountList[accountList.length - 1];

  return `No Logging Status report has run in the past 30 days for accounts ${allButLast}, and ${last}.`;
};

const createLoggingStatusRecommendation = ({ now, missingProfiles }) => {
  const evaluatedAt = now.toISOString();
  const reason = createReason(missingProfiles);
  const targetResources = mapMissingProfilesToTargets(missingProfiles);

  return {
    recommendationId: 'local_rule.logging_status_report',
    id: 'local_rule.logging_status_report',
    ruleId: RULE_ID,
    title: 'Run Logging Status Report',
    status: 'open',
    priority: 75,
    metadata: {
      priority: 75,
    ...
    recommendedAction: {
      type: 'report',
      reportId: LOGGING_REPORT_ID,
      label: 'Run Logging Status Report',
      sourceBlueprintId: LOGGING_REPORT_ID,
    },
    action: {
      type: 'report',
      reportId: LOGGING_REPORT_ID,
    ...
    targetResources,
    notes:
      'Confirm logging is enabled across AWS services to improve visibility and auditing.',
    createdAt: evaluatedAt,
    updatedAt: evaluatedAt,
  };
};

export const evaluateLoggingStatusReportRule = (
  userProfile,
  now = new Date()
) => {
  ...
  accountScans.forEach((scan) => {
    // Only process scans that are reports matching the logging status report ID
    if (!scan.reportId || scan.reportId !== LOGGING_REPORT_ID) {
      return;
    }
  ...
  return [
    createLoggingStatusRecommendation({
      now,
      missingProfiles,
    }),
  ];
...
export const loggingStatusReportRuleId = RULE_ID;
