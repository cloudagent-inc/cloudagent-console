...
const RULE_ID = 'resiliency_ha_report';
const RESILIENCY_REPORT_ID = 'report_aws_resiliency';
...
const createReason = (missingProfiles) => {
  if (!missingProfiles.length) {
    return 'No Resiliency and HA Best Practices report has run in the past 30 days.';
  }
  ...
  if (!accountList.length) {
    return 'No Resiliency and HA Best Practices report has run in the past 30 days for your connected environments.';
  }

  if (accountList.length === 1) {
    return `No Resiliency and HA Best Practices report has run in the past 30 days for account ${accountList[0]}.`;
  }

  if (accountList.length === 2) {
    return `No Resiliency and HA Best Practices report has run in the past 30 days for accounts ${accountList[0]} and ${accountList[1]}.`;
  }

  const allButLast = accountList.slice(0, -1).join(', ');
  const last = accountList[accountList.length - 1];

  return `No Resiliency and HA Best Practices report has run in the past 30 days for accounts ${allButLast}, and ${last}.`;
};

const createResiliencyRecommendation = ({ now, missingProfiles }) => {
  const evaluatedAt = now.toISOString();
  const reason = createReason(missingProfiles);
  const targetResources = mapMissingProfilesToTargets(missingProfiles);

  return {
    recommendationId: 'local_rule.resiliency_ha_report',
    id: 'local_rule.resiliency_ha_report',
    ruleId: RULE_ID,
    title: 'Run Resiliency and HA Best Practices Report',
    status: 'open',
    priority: 75,
    metadata: {
      priority: 75,
      domain: 'Operations & Reliability',
      category: 'Resiliency & Availability',
      ruleId: RULE_ID,
    ...
    recommendedAction: {
      type: 'report',
      reportId: RESILIENCY_REPORT_ID,
      label: 'Run Resiliency and HA Best Practices Report',
      sourceBlueprintId: RESILIENCY_REPORT_ID,
    },
    action: {
      type: 'report',
      reportId: RESILIENCY_REPORT_ID,
    ...
    targetResources,
    notes:
      'Validate high availability settings like multi-AZ deployments and delete protection across AWS services.',
    createdAt: evaluatedAt,
    updatedAt: evaluatedAt,
  };
};

export const evaluateResiliencyReportRule = (
  userProfile,
  now = new Date()
) => {
  ...
  accountScans.forEach((scan) => {
    // Only process scans that are reports matching the resiliency report ID
    if (!scan.reportId || scan.reportId !== RESILIENCY_REPORT_ID) {
      return;
    }
  ...
  return [
    createResiliencyRecommendation({
      now,
      missingProfiles,
    }),
  ];
...
export const resiliencyReportRuleId = RULE_ID;
