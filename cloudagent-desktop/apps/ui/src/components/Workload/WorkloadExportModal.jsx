...
function WorkloadExportModal({
  ...
  const handleExport = useCallback(async () => {
    ...
    const buildDocument = (documentOptions, image, dataOverrides = {}) => (
      <WorkloadExportPDFDocument
        ...
        recommendations={dataOverrides.recommendations ?? recommendations}
    ...
    const runPdfDiagnostics = async (originalError) => {
      ...
      const cases = [
        ...
        {
          label: 'recommendations first 5',
          options: { ...baseOptions, includeRecommendations: true },
          data: { recommendations: recommendations.slice(0, 5) },
        },
        {
          label: 'recommendations all',
          options: { ...baseOptions, includeRecommendations: true },
        },
        {
        ...
        {
          label: 'summary + recommendations + reports',
          options: {
            ...baseOptions,
            includeExecutiveSummary: true,
            includeRecommendations: true,
      ...
      console.info('[Workload PDF Export] Input sizes:', {
        ...
        recommendationJsonCharacters: getJsonSize(recommendations),
  ...
  }, [workload, executiveSummary, accountScans, reports, permissionProfiles, recommendations, options, hasAtLeastOneSection, onOpenChange, diagramNodes, diagramReady, captureDiagramImage]);
        ...
        <div className="space-y-4 py-4">
          ...
          >
            <div className="flex items-center gap-3">
              <div
                className={`p-2 rounded-lg ${
                  options.includeRecommendations ? 'bg-primary-100' : 'bg-gray-100'
                }`}
              >
                <Lightbulb
                  className={`w-4 h-4 ${
                    options.includeRecommendations ? 'text-primary-600' : 'text-gray-500'
                  }`}
                />
              </div>
              <div>
                ...
                <p className="text-xs text-gray-500">
                  {counts.recommendations > 0
                    ? `${counts.recommendations} recommendation${counts.recommendations !== 1 ? 's' : ''} for your workload`
            ...
            <Switch
              ...
              onCheckedChange={() => handleToggle('includeRecommendations')}
              className="data-[state=checked]:bg-primary-500"
            />
          </div>
        </div>
