...
import { sendemail } from '../../api/apigw';
import {
  BarChart3,
  Code2,
  Briefcase,
...
const OnboardingSurveyModal = ({ isOpen, onComplete, userEmail = '' }) => {
  ...
  const handleComplete = () => {
    ...
    const topicLabels = selectedTopics
      .map((t) => TOPIC_OPTIONS.find((opt) => opt.value === t)?.label || t)
      .join(', ');
    const platformLabels = selectedPlatforms
      .map((p) => PLATFORM_OPTIONS.find((opt) => opt.value === p)?.label || p)
      .join(', ');

    let emailMessage = 'CloudAgent Onboarding Survey Response\n';
    emailMessage += '=====================================\n\n';
    emailMessage += `User Email: ${userEmail || 'Unknown'}\n`;
    emailMessage += `Role: ${roleLabel}\n`;
    emailMessage += `Platforms: ${platformLabels || 'None selected'}\n`;
    emailMessage += `Topics of Interest: ${topicLabels || 'None selected'}\n`;
    emailMessage += `MSP/Consultant: ${isConsultant ? 'Yes' : 'No'}\n`;
    emailMessage += `Booked Demo: ${hasBookedDemo ? 'Yes' : 'No'}\n`;
    if (trimmedFeedback) {
      emailMessage += `\nWhat brought them to CloudAgent:\n${trimmedFeedback}\n`;
    }

    sendemail(emailMessage, 'CloudAgent Onboarding Survey Response');
