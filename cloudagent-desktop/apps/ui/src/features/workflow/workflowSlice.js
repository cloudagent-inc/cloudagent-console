...
const getWorkflowErrorDetails = (error) => {
  const workflowErrors = Array.isArray(error?.errors) ? error.errors : [];
  const messages = workflowErrors
    .map((workflowError) => workflowError?.message)
    .filter(Boolean);
...
export const getWorkflows = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      const errorDetails = getWorkflowErrorDetails(error);

      console.error('[workflowSlice.getWorkflows] Failed to load workflows', {
        variables: {
          count,
          nextToken,
          sortBy,
