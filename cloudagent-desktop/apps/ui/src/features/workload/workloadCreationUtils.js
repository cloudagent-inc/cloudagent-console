import { getGlobalWorkloadSecurityRules } from '@/components/SecurityRules/securityRulesUtils';
import { sanitizeGithubGovernance } from '@/components/SourceControlGovernance/githubGovernance';
...
export const sanitizeGlobalWorkloadDeploymentPreferences = (preferences = {}) => {
  const source = preferences && typeof preferences === 'object' ? preferences : {};
  const defaults = buildDefaultDeploymentPreferences();
  const supportedMethods = new Set(['cloudformation', 'terraform', 'opentofu']);
  const method = supportedMethods.has(source.method) ? source.method : defaults.method;

  return {
    method,
    changeSet: source.changeSet !== undefined ? !!source.changeSet : defaults.changeSet,
    changeSetNotifications: {
    ...
    requiredTags: Array.isArray(source.requiredTags) ? source.requiredTags : defaults.requiredTags,
    // GitHub source-control governance is the base (global) layer, so it is
    // stored as a fully-populated secure config rather than a partial override.
    github: sanitizeGithubGovernance(source.github),
    architecturePreferences: {
...
export const getGlobalWorkloadDeploymentPreferences = (settings = {}) => {
  ...
  const globalDefaults = sanitizeGlobalWorkloadDeploymentPreferences(source);
  return {
    ...defaults,
    ...globalDefaults,
    github: sanitizeGithubGovernance(globalDefaults.github),
    changeSetNotifications: {
      ...defaults.changeSetNotifications,
      ...(globalDefaults.changeSetNotifications || {}),
      email: {
        ...defaults.changeSetNotifications.email,
        ...(globalDefaults.changeSetNotifications?.email || {}),
      },
      slack: {
        ...defaults.changeSetNotifications.slack,
        ...(globalDefaults.changeSetNotifications?.slack || {}),
      },
    },
    architecturePreferences: {
      ...defaults.architecturePreferences,
      ...(globalDefaults.architecturePreferences || {}),
    },
  };
};
