...
import { fetchAgentConnection as fetchAgentConnectionApi } from '../../api/agent';
...
import {
  ...
  updateAgentConnectionWithoutAuthProfileMutation,
  updateUserAgentMutation,
  ...
  async (
    ...
    try {
      ...
      const response = await client.graphql({
        query: recordAgentConnectionMutation,
        variables: {
          itemId,
          agentType: normalizedAgentType,
          status,
      ...
      if (response.data.recordAgentConnection) {
       
        dispatch(
          updateUserProfile({
            agentHistory: {
              userId: response.data.recordAgentConnection.userId,
              recordId: response.data.recordAgentConnection.recordId,
              itemId: response.data.recordAgentConnection.itemId,
              agentType: response.data.recordAgentConnection.agentType,
              status: response.data.recordAgentConnection.status,
              purchaseDate: response.data.recordAgentConnection.purchaseDate,
              parentId: response.data.recordAgentConnection.parentId,
              title: response.data.recordAgentConnection.title,
              log: response.data.recordAgentConnection.log,
              authProfile: response.data.recordAgentConnection.authProfile,
            },
          })
        );
      }

      return response.data.recordAgentConnection;
    } catch (error) {
...
export const updateAgentConnection = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      const hasAuthProfile = authProfile !== undefined && authProfile !== null;
      const variables = {
        recordId,
        ...(status !== undefined ? { status } : {}),
        ...(log !== undefined ? { log } : {}),
      ...
      const response = await client.graphql({
        query: hasAuthProfile
          ? updateAgentConnectionMutation
          : updateAgentConnectionWithoutAuthProfileMutation,
        variables,
      });

      const updatedRecord = response?.data?.updateAgentConnection?.recordId
        ? response.data.updateAgentConnection
        : localUpdate;
...
export const cancelAgentConnection = createAsyncThunk(
  ...
  async ({ recordId, log }, { dispatch, rejectWithValue }) => {
    try {
      ...
      const response = await client.graphql({
        query: cancelAgentConnectionMutation,
        variables: {
          recordId,
          ...(log !== undefined ? { log } : {}),
        },
      ...
      const updatedRecord =
        response?.data?.cancelAgentConnection?.recordId
          ? response.data.cancelAgentConnection
          : {
              recordId,
              status: 'cancelled',
              ...(log !== undefined ? { log } : {}),
...
export const updateUserCredits = createAsyncThunk(
  ...
  async ({ credits, idempotencyKey, source = 'frontend', resourceType = 'legacy', resourceId = null, runId = null, metadata = {} }, { dispatch, rejectWithValue }) => {
    try {
      ...
      const restOperation = post({
        ...
        options: {
          body: {
            ...
            idempotencyKey:
              idempotencyKey ||
              `frontend:${source}:${resourceType}:${resourceId || 'unknown'}:${Date.now()}`,
            source,
            resourceType,
            resourceId,
            runId,
      ...
      const agentCredits = response.agentCredits;
...
export const refreshUserCredits = createAsyncThunk(
  ...
  async (_, { dispatch, rejectWithValue }) => {
    try {
      ...
      if (!response?.ok) {
        throw new Error(response?.error || 'Failed to refresh credits');
      }

      dispatch(
        updateUserProfile({
          agentCredits: response.agentCredits,
        })
      );

      return response.agentCredits;
    } catch (error) {
      console.log('error', error);
      return rejectWithValue(error.message);
    }
...
export const getAgentCount = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      const response = await client.graphql({
        query: queryGetAgentCount,
      });

      return response.data.getAgentCount;
    } catch (error) {
      return rejectWithValue(error.message);
    }
  }
...
export const getAgentConnection = createAsyncThunk(
  ...
  async (recordId, { rejectWithValue }) => {
    try {
      ...
      const response = await fetchAgentConnectionApi(recordId);
...
export const updateUserAgent = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      const updatedUser = response.data.updateUserAgent;
