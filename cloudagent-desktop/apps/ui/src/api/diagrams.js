...
export const DIAGRAMS_BASE_URL = BACKEND_API_ENDPOINT;
...
const getIdToken = async () => {
  try {
    ...
    console.warn('[Diagrams API] Failed to fetch auth session', error);
    return undefined;
  }
};
...
async function fetchJson(endpoint, { method = 'GET', body } = {}) {
  ...
  const idToken = runtimeLocal ? undefined : await getIdToken();
  const resolvedEndpoint = runtimeLocal
    ? getRuntimeApiUrl(endpoint.replace(DIAGRAMS_BASE_URL, ''), {
        fallbackApiBaseUrl: DIAGRAMS_BASE_URL,
      })
    : endpoint;
