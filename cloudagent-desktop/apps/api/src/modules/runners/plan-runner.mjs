import { executeLocalAwsCliCommand } from "../cloudagent/cloudagent-tools.mjs";
import {
  runLocalCodingAgentBlueprint,
} from "../skills/codex-runner.mjs";
import {
...
function localRunnerLog(message, details = {}) {
  const sanitized = { ...details };
  delete sanitized.authProfile;
  delete sanitized.credentials;
  console.log(`[local runner] ${message}`, sanitized);
}
...
function summarizeCliOutput(result = {}) {
  const stdout = safeTrim(result?.stdout ?? result?.output?.stdout ?? result?.output);
  const stderr = safeTrim(result?.stderr ?? result?.output?.stderr);
...
async function fetchLibraryBlueprint(blueprintId) {
  ...
  for (const url of [...new Set(candidates)]) {
    try {
      ...
      console.warn("[local workflow] failed to fetch library skill", {
        blueprintId: id,
        url,
...
function extractAwsCliOutputsFromContextEvents(contextEvents = []) {
  return contextEvents
    .filter((event) => event?.type === "tool_execution" && event?.sourceTool === "cli_session_execute")
    .map((event) => {
      ...
      return {
        ...
        statusCode: result.statusCode || (result.ok === false || output.ok === false ? 400 : 200),
      };
    });
}
...
function buildWorkflowTaskPrompt({
  ...
  return [
    `Execute this CloudAgent workflow skill task locally: ${task.title || task.id}.`,
    "",
    "Use the available local CloudAgent tools for real read-only AWS inspection when the task requires AWS data.",
    "If AWS data is needed, use cli_session_start and cli_session_execute with concrete AWS CLI commands instead of only describing what should happen.",
    "Run commands through the CloudAgent CLI session tools so the local runner can capture terminal evidence and session files.",
    "",
    "# Workflow Blueprint",
    JSON.stringify({
      title: runTitle,
...
async function runLocalCloudAgentTask({
  ...
  const contextEvents = [];
  const { tools } = createCloudAgentTools({ store, selectedAuthProfile: authProfile });
  const { stream } = await runCloudAgentStream({
    ...
    onContextEvent: (payload) => {
      ...
      if (payload && typeof onContextEvent === "function") onContextEvent(payload);
    },
  });
...
function buildLocalAgentBlueprintGoal({ title, blueprint = null, planPayload = null, plan = [] } = {}) {
  const parsedBlueprintPlan = blueprint ? parseJsonMaybe(blueprint.plan, {}) : {};
  const source = planPayload || parsedBlueprintPlan || {};
  const pieces = [
    title,
    source.title,
    source.description,
  ...
  const taskLines = [];
  for (const phase of Array.isArray(plan) ? plan : []) {
    const phaseTitle = firstNonEmpty(phase?.title, phase?.name);
    for (const task of asArray(phase?.tasks)) {
      taskLines.push([
        phaseTitle ? `${phaseTitle}:` : null,
        firstNonEmpty(task?.title, task?.name, task?.id, task?.task_id),
        safeTrim(task?.description).slice(0, 500) || null,
      ].filter(Boolean).join(" "));
    }
  }
  return [
    ...pieces.map((piece) => safeTrim(piece)).filter(Boolean),
    taskLines.length ? `Tasks:\n${taskLines.map((line) => `- ${line}`).join("\n")}` : null,
  ].filter(Boolean).join("\n\n");
}

function buildLocalAgentSessionContents({ plan = [], logs = [] } = {}) {
  const taskLabels = new Map();
  for (const phase of Array.isArray(plan) ? plan : []) {
    const phaseTitle = firstNonEmpty(phase?.title, phase?.name);
    for (const task of asArray(phase?.tasks)) {
      const id = firstNonEmpty(task?.id, task?.task_id);
      if (!id) continue;
      taskLabels.set(id, {
        title: firstNonEmpty(task?.title, task?.name, id),
        phaseTitle,
        description: safeTrim(task?.description).slice(0, 800) || null,
      });
    }
  }

  return (Array.isArray(logs) ? logs : []).map((entry) => {
    const label = taskLabels.get(entry?.taskId) || {};
    const commands = asArray(entry?.cli_command_output).map((commandOutput) => ({
      command: commandOutput?.command || commandOutput?.cli_command || "",
      statusCode: commandOutput?.statusCode ?? null,
    }));
    return {
      taskId: entry?.taskId || null,
      phaseIndex: entry?.phaseIndex ?? null,
      taskIndex: entry?.taskIndex ?? null,
      phaseTitle: label.phaseTitle || null,
      taskTitle: label.title || entry?.taskTitle || entry?.title || entry?.taskId || null,
      taskDescription: label.description || null,
      status: entry?.status || null,
      output: safeTrim(entry?.task_output || entry?.output).slice(0, 6000),
      evidence: {
        commandCount: commands.length,
        commands,
      },
      toolExecutions: asArray(entry?.toolExecutions).map((tool) => ({
        name: tool?.name || tool?.toolName || null,
...
async function buildLocalCloudAgentRunSummaryWithOpenAI({
  title,
  status,
  blueprint = null,
  planPayload = null,
  ...
  const fallbackText = fallbackSummary?.finalSummary || fallbackSummary?.summary || "";
  try {
    const blueprintGoal = buildLocalAgentBlueprintGoal({ title, blueprint, planPayload, plan });
    const sessionContents = buildLocalAgentSessionContents({ plan, logs });
    localRunnerLog("local CloudAgent run summary LLM starting", {
      title,
      status,
      taskCount: Array.isArray(sessionContents) ? sessionContents.length : 0,
      blueprintGoalChars: blueprintGoal.length,
      fallbackSummaryChars: fallbackText.length,
    });
    const llmSummary = await generateLocalAgentSessionSummaryWithOpenAI({
      title,
      runner: "CloudAgent",
      status,
      blueprintGoal,
    ...
    const text = safeTrim(llmSummary);
    if (!text) {
      localRunnerLog("local CloudAgent run summary LLM returned empty summary", {
        title,
        status,
      });
      return fallbackSummary;
    }
    localRunnerLog("local CloudAgent run summary LLM completed", {
      title,
      status,
    ...
    return {
      ...
      finalSummary: text,
      rawFinalSummary: fallbackText,
      generatedAt: nowIso(),
      generatedBy: "local-openai",
      status,
    };
  } catch (error) {
    localRunnerLog("local CloudAgent run summary generation failed", {
      title,
      status,
...
export async function executeLocalAgentPlanWithCloudAgent({
  ...
  for (let phaseIndex = 0; phaseIndex < plan.length; phaseIndex += 1) {
    ...
    for (let taskIndex = 0; taskIndex < phase.tasks.length; taskIndex += 1) {
      ...
      try {
        localRunnerLog("local CloudAgent task stream starting", {
        ...
        const result = await runLocalCloudAgentTask({
          ...
          onToken: typeof onTaskToken === "function"
            ...
            : null,
          onContextEvent: typeof onContextEvent === "function"
            ? (payload) => onContextEvent({
                payload,
                task,
                phase,
                phaseIndex,
        ...
        output = result.text || "";
        localRunnerLog("local CloudAgent task stream finished", {
          workflowRunId: parentId || null,
          agentRunId: runRecord.recordId,
          blueprintId: resolvedPlanId,
          taskId: task.id,
        ...
        if (!output) {
          ...
          output = [
            output,
            "",
            "Local runner did not record any cli_session_execute tool execution for this task, so it was marked failed instead of completed.",
          ].join("\n");
        }
      } catch (error) {
        status = "failed";
        output = error?.message || String(error);
        localRunnerLog("local CloudAgent task stream failed", {
          workflowRunId: parentId || null,
          agentRunId: runRecord.recordId,
          blueprintId: resolvedPlanId,
          taskId: task.id,
  ...
  const finalStatus = failedCount ? "failed" : "complete";
  const deterministicFinalSummary = buildLocalCloudAgentRunSummary({
    title: runTitle,
    status: finalStatus,
    plan,
    logs,
  ...
  const finalSummary = await buildLocalCloudAgentRunSummaryWithOpenAI({
    title: runTitle,
    status: finalStatus,
    blueprint,
    planPayload,
    plan,
    logs,
    fallbackSummary: deterministicFinalSummary,
  });
  const finalRecord = await store.updateAgentHistoryRecord(runRecord.recordId, {
    status: finalStatus,
    log: {
...
async function executeLocalAgentPlanWithCodex({
  ...
  localRunnerLog(`local ${runnerLabel} workflow task starting`, {
    workflowRunId: parentId || null,
    agentRunId: runRecord.recordId,
    blueprintId: resolvedPlanId,
    title: runTitle,
  ...
  localRunnerLog(`local ${runnerLabel} workflow task finished`, {
    workflowRunId: parentId || null,
    agentRunId: finalRecord.recordId,
    blueprintId: resolvedPlanId,
    status: finalStatus,
...
function executeDecisionWorkflowNode({ node, nodeIndex = 0, executionHistory = [] } = {}) {
  ...
  localRunnerLog("decision node evaluated", {
    nodeId: node.id,
    nodeName: node.name,
    nodeIndex,
    selectedNextIds,
...
async function executeWorkflowReportNode({ store, node, workflowRunId, nodeIndex = 0 }) {
  ...
  localRunnerLog("report node started", {
    workflowRunId,
    nodeId: node.id,
    nodeName: node.name,
    nodeIndex,
  ...
  if (!reportType) {
    ...
    localRunnerLog("report node failed", {
      workflowRunId,
      nodeId: node.id,
  ...
  try {
    ...
    localRunnerLog("report node finished", {
      workflowRunId,
      nodeId: node.id,
      reportType,
      status,
...
async function executeWorkflowTaskNode({
  ...
  localRunnerLog("task node started", {
    workflowRunId,
    nodeId: node.id,
    nodeName: node.name,
    nodeType: node.type,
  ...
  for (const blueprintId of blueprintIds) {
    ...
    if (blueprintId && !blueprint) {
      tasks.push({
        ...
        updatedAt: nowIso(),
        completedAt: nowIso(),
        output: `Skill ${blueprintId} is not available in local files.`,
        taskInputSettings: node.inputSettings,
        result: workflowTaskResult({
          status: "failed",
          message: `Skill ${blueprintId} is not available in local files.`,
          extra: { blueprintId, error: "BLUEPRINT_NOT_FOUND" },
        }),
      });
      localRunnerLog("task failed", {
        workflowRunId,
        nodeId: node.id,
        taskId,
    ...
    if (credentialBlocker) {
      ...
      localRunnerLog("task blocked by credential status", {
        workflowRunId,
        nodeId: node.id,
        taskId,
        blueprintId: blueprintId || null,
    ...
    localRunnerLog(`task dispatching local ${
    ...
    if (!llmResult) {
      localRunnerLog("task falling back to local CLI extractor", {
        workflowRunId,
        nodeId: node.id,
        taskId,
        blueprintId: blueprintId || null,
    ...
    const result = llmResult || await executeAgentPlan({
      store,
      planId: blueprintId || node.id,
      blueprint,
      planPayload: syntheticPlanPayload,
  ...
  localRunnerLog("task node finished", {
    workflowRunId,
    nodeId: node.id,
    nodeName: node.name,
    nodeIndex,
...
async function executeWorkflowCommunicationNode({
  ...
  localRunnerLog("communication node started", {
    workflowRunId,
    nodeId: node.id,
    nodeName: node.name,
    nodeIndex,
  ...
  try {
    ...
    localRunnerLog("communication email generation failed", {
      workflowRunId,
      nodeId: node.id,
  ...
  localRunnerLog("communication node dummy email recorded", {
    workflowRunId,
    nodeId: node.id,
    nodeName: node.name,
    nodeIndex,
...
function executeSimpleWorkflowNode(node, nodeIndex = 0) {
  ...
  localRunnerLog("simple node executed", {
    nodeId: node.id,
    nodeName: node.name,
    nodeType: node.type,
    nodeIndex,
...
async function buildFinalWorkflowRunSummary({
  ...
  try {
    ...
    localRunnerLog("workflow summary generation failed", {
...
export async function executeLocalWorkflow({
  ...
  localRunnerLog("workflow started", {
    workflowRunId: initialRun.workflowRunId,
    workflowId: definition.workflowId || null,
    title: definition.title,
  ...
  for (let nodeIndex = 0; nodeIndex < orderedNodes.length; nodeIndex += 1) {
    ...
    if (String(latestRun?.workflowStatus || "").toLowerCase() === "cancelled") {
      ...
      localRunnerLog("workflow stopped before next node", {
        workflowRunId: initialRun.workflowRunId,
        stopReason,
        nodeId: node.id,
        nodeName: node.name,
    ...
    if (skippedNodeIds.has(node.id)) {
      localRunnerLog("node skipped by decision branch", {
        workflowRunId: initialRun.workflowRunId,
        nodeId: node.id,
        nodeName: node.name,
        nodeType: node.type,
    ...
    if (String(latestAfterNodeRun?.workflowStatus || "").toLowerCase() === "cancelled") {
      stoppedEarly = true;
      stopReason = "workflow_cancelled";
      localRunnerLog("workflow stopped after node because it was cancelled", {
        workflowRunId: initialRun.workflowRunId,
        stopReason,
        nodeId: node.id,
        nodeName: node.name,
    ...
    if (node.type === "decision") {
      ...
      localRunnerLog("decision branch selection applied", {
        workflowRunId: initialRun.workflowRunId,
        nodeId: node.id,
        selectedNextIds: result.selectedNextIds || [],
        skippedNextIds: result.skippedNextIds || [],
    ...
    localRunnerLog("workflow status updated", {
      workflowRunId: initialRun.workflowRunId,
      workflowStatus: nextStatus,
    ...
    if (nodeFailed || nodeWaiting) {
      ...
      localRunnerLog("workflow stopped before downstream nodes", {
        workflowRunId: initialRun.workflowRunId,
        stopReason,
        nodeId: node.id,
        nodeName: node.name,
  ...
  localRunnerLog("workflow finished", {
