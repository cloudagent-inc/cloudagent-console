...
function resolveExecutablePath(command, env = process.env) {
  ...
  const candidates =
    path.isAbsolute(binary) || binary.includes(path.sep)
      ? [path.resolve(binary)]
...
export function resolveCodexCodeModeHostPath(command, env = process.env) {
  const configuredHost = String(env?.CODEX_CODE_MODE_HOST_PATH || "").trim();
  if (configuredHost) return configuredHost;
  const executablePath = resolveExecutablePath(command, env);
  if (!executablePath) return null;
  let realExecutablePath;
  try {
    realExecutablePath = fsSync.realpathSync(executablePath);
  } catch {
    return null;
  }
  const hostPath = path.join(path.dirname(realExecutablePath), "codex-code-mode-host");
  try {
    fsSync.accessSync(hostPath, fsSync.constants.X_OK);
    return hostPath;
  } catch {
    return null;
  }
...
function buildCodexRuntimeEnv(command, env = {}) {
  const inheritedEnv = { ...process.env, ...(env || {}) };
  const codeModeHostPath = resolveCodexCodeModeHostPath(command, inheritedEnv);
  return codeModeHostPath
    ? { ...(env || {}), CODEX_CODE_MODE_HOST_PATH: codeModeHostPath }
    : env;
}
...
function getClaudeExecOptions(prompt, { mcpConfigPath = null, continuePrevious = false } = {}) {
  ...
  const permissionMode = String(
    process.env.CLOUDAGENT_CLAUDE_PERMISSION_MODE || (mcpConfigPath ? "bypassPermissions" : "")
  ).trim();
  if (permissionMode) {
...
export async function runLocalCodingAgentBlueprint({
  ...
  const prompt = [
    ...
    agentRunner === "claude" && mcpUrl
      ? "- CloudAgent pre-approves the CloudAgent MCP tools for this non-interactive Claude session. Do not tell the user to approve a CloudAgent permission prompt; call the tool directly. If a tool is actually rejected or unavailable, report the exact error."
      : null,
  ...
  const runtimeEnv =
    agentRunner === "codex" ? buildCodexRuntimeEnv(command, awsCredentialEnv) : awsCredentialEnv;
  ...
  const launchDebug = {
    ...
    codexCodeModeHostPath: runtimeEnv.CODEX_CODE_MODE_HOST_PATH || null,
    timeoutMs,
  };
...
export async function resumeLocalCodexBlueprint({
  ...
  const runtimeEnv = buildCodexRuntimeEnv(command, awsCredentialEnv);
  const launchDebug = {
    type: "agent_runtime_debug",
    stage: "process_resume",
    runner: "codex",
