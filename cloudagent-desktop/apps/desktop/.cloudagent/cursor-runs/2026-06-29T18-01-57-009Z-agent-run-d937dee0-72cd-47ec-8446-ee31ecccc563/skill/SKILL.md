...
Collect AWS Backup backup jobs created in the last 30 days in account 025825898506 and region us-east-1, using the provided default review window without prompting, and identify any jobs with failed, expired, aborted, completed-with-issues, or other non-success statuses.
...
- Use cli_session_command_execute to compute and fix the review window from the provided defaults without prompting because execution_preferences.useDefaultValuesWithoutConfirmation is true: set review_window_start_timestamp to 30 days before assessment time and review_window_end_timestamp to assessment time.
- Use cli_session_command_execute to verify the active AWS caller identity and region context for account 025825898506 in us-east-1 before collecting data.
- Use cli_session_command_execute to call AWS Backup list-backup-jobs in us-east-1 and paginate as needed to collect backup jobs created between the computed review_window_start_timestamp and review_window_end_timestamp.
- Use cli_session_command_execute to normalize the returned backup job records and capture job ID, state/status fields, creation time, start time, completion time, protected resource identifiers, vault fields, and diagnostic message fields returned by AWS Backup.
- Use cli_session_command_execute to filter the collected backup jobs to only non-success statuses, including failed, expired, aborted, completed-with-issues, or any status outside the expected success state.
- Use cli_session_command_execute to record explicit not-applicable values for destination vault, destination Region, destination account, and restore target details when AWS Backup does not provide those fields for backup jobs.
- Produce a workload-independent assessment result for backup jobs in scope that can be aggregated with restore and copy job findings in the summary task.
...
Collect AWS Backup restore jobs created in the last 30 days in account 025825898506 and region us-east-1, using the provided default review window without prompting, and identify any jobs with failed, expired, aborted, completed-with-issues, or other non-success statuses.
...
- Use cli_session_command_execute to compute and fix the review window from the provided defaults without prompting because execution_preferences.useDefaultValuesWithoutConfirmation is true: set review_window_start_timestamp to 30 days before assessment time and review_window_end_timestamp to assessment time.
- Use cli_session_command_execute to verify the active AWS caller identity and region context for account 025825898506 in us-east-1 before collecting data.
- Use cli_session_command_execute to call AWS Backup list-restore-jobs in us-east-1 and paginate as needed to collect restore jobs created between the computed review_window_start_timestamp and review_window_end_timestamp.
- Use cli_session_command_execute to normalize the returned restore job records and capture job ID, status, timestamps, source recovery point or protected resource identifiers, source vault, restore metadata, target details, destination Region/account fields if present, and diagnostic message fields returned by AWS Backup.
- Use cli_session_command_execute to filter the collected restore jobs to only non-success statuses, including failed, expired, aborted, completed-with-issues, or any status outside the expected success state.
- Use cli_session_command_execute to record explicit not-applicable or not-returned values for destination vault, destination Region, destination account, and other fields when AWS Backup does not provide them for a restore job.
- Produce a workload-independent assessment result for restore jobs in scope that can be aggregated with backup and copy job findings in the summary task.
...
Collect AWS Backup copy jobs created in the last 30 days in account 025825898506 and region us-east-1, using the provided default review window without prompting, and identify any jobs with failed, expired, aborted, completed-with-issues, or other non-success statuses.
...
- Use cli_session_command_execute to compute and fix the review window from the provided defaults without prompting because execution_preferences.useDefaultValuesWithoutConfirmation is true: set review_window_start_timestamp to 30 days before assessment time and review_window_end_timestamp to assessment time.
- Use cli_session_command_execute to verify the active AWS caller identity and region context for account 025825898506 in us-east-1 before collecting data.
- Use cli_session_command_execute to call AWS Backup list-copy-jobs in us-east-1 and paginate as needed to collect copy jobs created between the computed review_window_start_timestamp and review_window_end_timestamp.
- Use cli_session_command_execute to normalize the returned copy job records and capture job ID, status, timestamps, source recovery point or protected resource identifiers, source vault, destination vault, destination Region, destination account if present, and diagnostic message fields returned by AWS Backup.
- Use cli_session_command_execute to filter the collected copy jobs to only non-success statuses, including failed, expired, aborted, completed-with-issues, or any status outside the expected success state.
- Use cli_session_command_execute to record explicit not-applicable values for restore target details when AWS Backup does not provide those fields for copy jobs, and explicit not-returned values for destination account when absent.
- Produce a workload-independent assessment result for copy jobs in scope that can be aggregated with backup and restore job findings in the summary task.
...
- Use the outputs from the three prior assessment tasks as inputs; no additional environment-wide discovery is required.
- Validate that each assessment used the provided default review window without prompting because execution_preferences.useDefaultValuesWithoutConfirmation is true: review_window_start_timestamp = 30 days before assessment time and review_window_end_timestamp = assessment time.
- Aggregate reviewed-job counts and non-success-job counts across backup, restore, and copy job types.
- Build a single combined result set and summary table containing all non-success jobs with job type, job ID, resource identifier, vault/source-destination context, Region/account context, restore target details where applicable, timestamps, and diagnostic fields.
- Mark fields as not applicable or not returned where appropriate so the combined report is unambiguous across job types.
- Summarize any recurring failure patterns found in the aggregated results, or explicitly state that no non-success jobs were found if the combined result set is empty.
- Produce the final read-only assessment summary for the user; no mutation tool is required.
