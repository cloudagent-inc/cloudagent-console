...
- If Cursor does not expose CloudAgent MCP tools in your direct tool list, call the configured CloudAgent MCP URL with JSON-RPC `tools/call` directly. The local CloudAgent MCP server accepts direct loopback `tools/call` requests for these tools without a prior MCP session ID.
...
Collect AWS Backup backup jobs created in the last 30 days in AWS account 025825898506 and region us-east-1, then identify any jobs with failed, expired, aborted, completed-with-issues, or other non-success statuses.
...
- Because execution_preferences.useDefaultValuesWithoutConfirmation is true, use the provided default review window values without prompting the user: review_window_start_timestamp = 30 days before assessment time and review_window_end_timestamp = assessment time.
- Use cli_session_command_execute to determine the concrete start and end timestamps for the review window at execution time.
- Use cli_session_command_execute to call AWS Backup list-backup-jobs for the review window in us-east-1 and collect all backup jobs created in scope.
- Use cli_session_command_execute to inspect the returned backup job details and isolate all jobs whose status is not the expected success state.
- Use cli_session_command_execute to collect for each non-success backup job the job ID, status, timestamps, protected resource identifier, source vault if present, and failure/status diagnostic fields.
- Document explicitly for each non-success backup job that destination vault, destination region, destination account, and restore target details are not applicable when AWS Backup does not return those fields for backup jobs.
- Prepare structured assessment output for downstream summary, including reviewed job counts and the detailed non-success backup job records or an explicit no-findings result.
...
Collect AWS Backup restore jobs created in the last 30 days in AWS account 025825898506 and region us-east-1, then identify any jobs with failed, expired, aborted, completed-with-issues, or other non-success statuses.
...
- For each non-success restore job, recorded the destination backup vault if applicable or explicitly noted that no destination backup vault applies.
...
- Because execution_preferences.useDefaultValuesWithoutConfirmation is true, use the provided default review window values without prompting the user: review_window_start_timestamp = 30 days before assessment time and review_window_end_timestamp = assessment time.
- Use cli_session_command_execute to determine the concrete start and end timestamps for the review window at execution time.
- Use cli_session_command_execute to call AWS Backup list-restore-jobs for the review window in us-east-1 and collect all restore jobs created in scope.
- Use cli_session_command_execute to inspect the returned restore job details and isolate all jobs whose status is not the expected success state.
- Use cli_session_command_execute to collect for each non-success restore job the restore job ID, status, timestamps, source recovery point or protected resource identifiers, source vault if present, destination region/account fields if returned, restore target details, and failure/status diagnostic fields.
- Document explicitly for each non-success restore job when destination backup vault, destination region, or destination account are not applicable or are not returned by AWS Backup.
- Prepare structured assessment output for downstream summary, including reviewed job counts and the detailed non-success restore job records or an explicit no-findings result.
...
Collect AWS Backup copy jobs created in the last 30 days in AWS account 025825898506 and region us-east-1, then identify any jobs with failed, expired, aborted, completed-with-issues, or other non-success statuses.
...
- Because execution_preferences.useDefaultValuesWithoutConfirmation is true, use the provided default review window values without prompting the user: review_window_start_timestamp = 30 days before assessment time and review_window_end_timestamp = assessment time.
- Use cli_session_command_execute to determine the concrete start and end timestamps for the review window at execution time.
- Use cli_session_command_execute to call AWS Backup list-copy-jobs for the review window in us-east-1 and collect all copy jobs created in scope.
- Use cli_session_command_execute to inspect the returned copy job details and isolate all jobs whose status is not the expected success state.
- Use cli_session_command_execute to collect for each non-success copy job the copy job ID, status, timestamps, source recovery point or protected resource identifiers, source backup vault, destination backup vault, destination region, destination account if returned, and failure/status diagnostic fields.
- Document explicitly for each non-success copy job that restore target details do not apply to copy jobs.
- Prepare structured assessment output for downstream summary, including reviewed job counts and the detailed non-success copy job records or an explicit no-findings result.
...
- Use the structured outputs from the three prior assessment tasks as the input dataset for the final summary.
- Validate that all assessment outputs cover the same default review window applied without user confirmation: from 30 days before assessment time through assessment time.
- Aggregate reviewed job totals and non-success job totals across backup, restore, and copy job types.
- Produce a combined table of all non-success jobs, preserving job type, identifiers, source/destination details, restore target details where applicable, timestamps, and diagnostic fields.
- Clearly label fields as not applicable or not returned where appropriate for each job type.
- Summarize any recurring failure patterns visible across the aggregated non-success jobs, or explicitly state that no patterns were observed because no non-success jobs were found.
- Produce a final account-and-region-scoped summary for AWS account 025825898506 in us-east-1 that is consistent with the detailed assessment findings.
