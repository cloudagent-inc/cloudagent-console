...
Collect AWS Backup backup jobs created in the last 30 days in AWS account 025825898506 and region us-east-1, using the provided default review window values without prompting, and identify any jobs with failed, expired, aborted, completed-with-issues, or other non-success statuses.
...
- Use cli_session_command_execute to confirm the active AWS caller identity and region context for account 025825898506 in us-east-1.
- Use cli_session_command_execute to set the review window from the provided default values without prompting because execution_preferences.useDefaultValuesWithoutConfirmation is true: review_window_start_timestamp = 30 days before assessment time, review_window_end_timestamp = assessment time.
- Use cli_session_command_execute to list AWS Backup backup jobs in us-east-1 for the last 30 days.
- Use cli_session_command_execute to retrieve detailed fields for the returned backup jobs as needed, including job ID, state/status, message/reason fields, protected resource identifiers, vault information, and timestamps.
- Use cli_session_command_execute to filter the backup jobs to only non-success statuses, including failed, expired, aborted, completed-with-issues, or any other status outside the expected success state.
- Use cli_session_command_execute to normalize the backup-job findings into a structured result set that explicitly marks non-applicable destination or restore-target fields for backup jobs.
- Document whether any non-success backup jobs were found and preserve the output for the final summary task.
...
Collect AWS Backup restore jobs created in the last 30 days in AWS account 025825898506 and region us-east-1, using the provided default review window values without prompting, and identify any jobs with failed, expired, aborted, completed-with-issues, or other non-success statuses.
...
- Use cli_session_command_execute to confirm the active AWS caller identity and region context for account 025825898506 in us-east-1.
- Use cli_session_command_execute to use the provided default review window values without prompting because execution_preferences.useDefaultValuesWithoutConfirmation is true: review_window_start_timestamp = 30 days before assessment time, review_window_end_timestamp = assessment time.
- Use cli_session_command_execute to list AWS Backup restore jobs in us-east-1 for the last 30 days.
- Use cli_session_command_execute to retrieve detailed fields for the returned restore jobs as needed, including job ID, status, recovery point or protected resource identifiers, restore metadata, message/reason fields, and timestamps.
- Use cli_session_command_execute to filter the restore jobs to only non-success statuses, including failed, expired, aborted, completed-with-issues, or any other status outside the expected success state.
- Use cli_session_command_execute to normalize the restore-job findings into a structured result set that includes target details and explicitly marks non-applicable or unavailable fields.
- Document whether any non-success restore jobs were found and preserve the output for the final summary task.
...
Collect AWS Backup copy jobs created in the last 30 days in AWS account 025825898506 and region us-east-1, using the provided default review window values without prompting, and identify any jobs with failed, expired, aborted, completed-with-issues, or other non-success statuses.
...
- Use cli_session_command_execute to confirm the active AWS caller identity and region context for account 025825898506 in us-east-1.
- Use cli_session_command_execute to use the provided default review window values without prompting because execution_preferences.useDefaultValuesWithoutConfirmation is true: review_window_start_timestamp = 30 days before assessment time, review_window_end_timestamp = assessment time.
- Use cli_session_command_execute to list AWS Backup copy jobs in us-east-1 for the last 30 days.
- Use cli_session_command_execute to retrieve detailed fields for the returned copy jobs as needed, including job ID, status, source and destination vault information, destination region/account details, message/reason fields, resource identifiers, and timestamps.
- Use cli_session_command_execute to filter the copy jobs to only non-success statuses, including failed, expired, aborted, completed-with-issues, or any other status outside the expected success state.
- Use cli_session_command_execute to normalize the copy-job findings into a structured result set that explicitly marks restore-target fields as not applicable for copy jobs.
- Document whether any non-success copy jobs were found and preserve the output for the final summary task.
...
Summarize the backup, restore, and copy job review results for the last 30 days in AWS account 025825898506 and region us-east-1 and state whether any non-success jobs were found or all jobs were successful.
...
- Use the preserved outputs from the three prior cli_session_command_execute assessment tasks as the only input set for the summary.
- Validate that all three assessments used the provided default review window values without prompting because execution_preferences.useDefaultValuesWithoutConfirmation is true: review_window_start_timestamp = 30 days before assessment time, review_window_end_timestamp = assessment time.
- Aggregate the counts of reviewed and non-success backup, restore, and copy jobs across the three result sets.
- Produce a combined table of all non-success jobs with the required identifying, source/destination, target, diagnostic, and timestamp fields, explicitly marking fields that are not applicable.
- Summarize recurring failure patterns if present, or explicitly state that no non-success jobs were found if the aggregated result set is empty.
- Return a final summary consistent with the detailed findings from the three assessment tasks.
