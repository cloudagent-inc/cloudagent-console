...
- Use cli_session_command_execute to calculate and apply the review window with default values without confirmation because execution_preferences.useDefaultValuesWithoutConfirmation is true: start timestamp = 30 days before assessment time, end timestamp = assessment time.
- Use cli_session_command_execute to run AWS CLI commands in region us-east-1 under account 025825898506 to list AWS Backup backup jobs created within the calculated review window.
- Use cli_session_command_execute to paginate through all backup job results in scope and retrieve detailed fields for each job, including job ID, state/status, message or status reason fields, resource identifiers, vault information, and timestamps.
- Use cli_session_command_execute to filter the collected backup jobs to only non-success statuses such as FAILED, EXPIRED, ABORTED, COMPLETED_WITH_ISSUES, or any other status outside the expected success state.
- Use cli_session_command_execute to normalize each non-success backup job into a structured result containing job type, job ID, protected resource ARN or identifier, source vault, destination vault applicability, destination region applicability, destination account applicability, restore target applicability, diagnostic message, and relevant timestamps.
- Document explicitly when no non-success backup jobs are found in the 30-day review window.
...
- Use cli_session_command_execute to calculate and apply the review window with default values without confirmation because execution_preferences.useDefaultValuesWithoutConfirmation is true: start timestamp = 30 days before assessment time, end timestamp = assessment time.
- Use cli_session_command_execute to run AWS CLI commands in region us-east-1 under account 025825898506 to list AWS Backup restore jobs created within the calculated review window.
- Use cli_session_command_execute to paginate through all restore job results in scope and retrieve detailed fields for each job, including job ID, status, recovery point or protected resource identifiers, source vault information if available, restore metadata, target details, diagnostic fields, and timestamps.
- Use cli_session_command_execute to filter the collected restore jobs to only non-success statuses such as FAILED, EXPIRED, ABORTED, COMPLETED_WITH_ISSUES, or any other status outside the expected success state.
- Use cli_session_command_execute to normalize each non-success restore job into a structured result containing job type, job ID, source recovery point ARN or protected resource identifier, source vault, destination vault applicability, destination region, destination account, restore target details, diagnostic message, and relevant timestamps.
- Document explicitly when no non-success restore jobs are found in the 30-day review window.
...
- Use cli_session_command_execute to calculate and apply the review window with default values without confirmation because execution_preferences.useDefaultValuesWithoutConfirmation is true: start timestamp = 30 days before assessment time, end timestamp = assessment time.
- Use cli_session_command_execute to run AWS CLI commands in region us-east-1 under account 025825898506 to list AWS Backup copy jobs created within the calculated review window.
- Use cli_session_command_execute to paginate through all copy job results in scope and retrieve detailed fields for each job, including job ID, status, source recovery point or protected resource identifiers, source and destination vaults, destination region, destination account if returned, diagnostic fields, and timestamps.
- Use cli_session_command_execute to filter the collected copy jobs to only non-success statuses such as FAILED, EXPIRED, ABORTED, COMPLETED_WITH_ISSUES, or any other status outside the expected success state.
- Use cli_session_command_execute to normalize each non-success copy job into a structured result containing job type, job ID, source recovery point ARN or protected resource identifier, source vault, destination vault, destination region, destination account, restore target non-applicability, diagnostic message, and relevant timestamps.
- Document explicitly when no non-success copy jobs are found in the 30-day review window.
...
- Use the outputs from the three prior assessment tasks; no additional discovery outside account 025825898506 and region us-east-1 is needed.
- Aggregate the reviewed-job counts and non-success-job counts across backup, restore, and copy job types for the same default review window of 30 days before assessment time through assessment time.
- Produce a combined table of all non-success jobs with job type, job ID, resource identifier, source vault, destination vault, destination region, destination account, restore target details, diagnostic message, and relevant timestamps.
- Mark fields as not applicable when a field does not apply to a given job type rather than leaving blanks with ambiguous meaning.
- Summarize recurring failure patterns if any exist, or explicitly state that no non-success jobs were found and that all reviewed jobs were successful or had no non-success statuses in scope.
