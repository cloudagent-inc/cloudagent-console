...
Collect AWS Backup backup, restore, and copy jobs created in the last 30 days in us-east-1 for account 025825898506 and identify any jobs with failed, expired, aborted, completed-with-issues, or other non-success statuses.
...
- Use cli_session_command_execute to set the AWS CLI region context to us-east-1 and verify the caller identity for account 025825898506 before collecting job data.
- Use cli_session_command_execute to calculate and use the default review window without prompting the user because execution_preferences.useDefaultValuesWithoutConfirmation is true: review_window_start_timestamp = 30 days before assessment time and review_window_end_timestamp = assessment time.
- Use cli_session_command_execute to list AWS Backup backup jobs in us-east-1 created within the default 30-day review window and collect job IDs, status fields, timestamps, protected resource identifiers, vault details, and diagnostic fields returned by AWS Backup.
- Use cli_session_command_execute to filter the backup jobs to only non-success statuses, including failed, expired, aborted, completed-with-issues, and any other status outside the expected success state, and record all required per-job details.
- Use cli_session_command_execute to list AWS Backup restore jobs in us-east-1 created within the default 30-day review window and collect job IDs, status fields, timestamps, recovery point or resource identifiers, source vault details, destination details if returned, restore target metadata, and diagnostic fields returned by AWS Backup.
- Use cli_session_command_execute to filter the restore jobs to only non-success statuses, including failed, expired, aborted, completed-with-issues, and any other status outside the expected success state, and record all required per-job details.
- Use cli_session_command_execute to list AWS Backup copy jobs in us-east-1 created within the default 30-day review window and collect job IDs, status fields, timestamps, recovery point or resource identifiers, source and destination vault details, destination Region and account details if returned, and diagnostic fields returned by AWS Backup.
- Use cli_session_command_execute to filter the copy jobs to only non-success statuses, including failed, expired, aborted, completed-with-issues, and any other status outside the expected success state, and record all required per-job details.
- Use cli_session_command_execute to verify whether any non-success jobs were found across backup, restore, and copy job types and prepare the structured assessment output for summary.
...
Summarize the AWS Backup backup, restore, and copy job review results for the last 30 days in us-east-1 for account 025825898506 and state whether any non-success jobs were found or all jobs were successful.
...
- Use the outputs from the consolidated assessment task and do not prompt the user for the review window because execution_preferences.useDefaultValuesWithoutConfirmation is true and the default window is already set to 30 days before assessment time through assessment time.
- Aggregate the reviewed job counts and non-success job counts across backup, restore, and copy job types.
- Produce a combined non-success jobs table with the required identifiers, source and destination details, timestamps, restore target details where applicable, and diagnostic messages.
- Summarize recurring failure patterns if any exist, or explicitly state that no non-success jobs were found and that reviewed jobs were successful or had no non-success statuses in scope.
- Validate that the final summary is internally consistent with the detailed findings from the assessment task before presenting the result.
