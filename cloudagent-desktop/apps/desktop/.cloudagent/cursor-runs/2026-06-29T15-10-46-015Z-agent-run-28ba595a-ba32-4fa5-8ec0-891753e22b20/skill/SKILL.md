...
- Because execution_preferences.useDefaultValuesWithoutConfirmation is true, use the provided default review window without prompting the user: review_window_start_timestamp = 30 days before assessment time and review_window_end_timestamp = assessment time.
- Use cli_session_command_execute to compute or derive the review window timestamps at execution time and list AWS Backup backup jobs in scope for that window.
- Use cli_session_command_execute to retrieve backup job details for each job returned in the window, including job ID, status, timestamps, protected resource identifiers, vault information, and diagnostic fields.
- Use cli_session_command_execute to filter backup jobs to non-success statuses and record all required fields, explicitly marking fields that do not apply to backup jobs.
- Prepare the backup-job review output so it can be aggregated with restore and copy job findings in the summary task.
...
- Use cli_session_command_execute to run AWS CLI commands against account 025825898506 in region us-east-1.
- Because execution_preferences.useDefaultValuesWithoutConfirmation is true, use the provided default review window without prompting the user: review_window_start_timestamp = 30 days before assessment time and review_window_end_timestamp = assessment time.
- Use cli_session_command_execute to compute or derive the review window timestamps at execution time and list AWS Backup restore jobs in scope for that window.
- Use cli_session_command_execute to retrieve restore job details for each job returned in the window, including job ID, status, timestamps, recovery point identifiers, protected resource identifiers, restore metadata, target details, and diagnostic fields.
- Use cli_session_command_execute to filter restore jobs to non-success statuses and record all required fields, explicitly marking fields that are not applicable or not returned.
- Prepare the restore-job review output so it can be aggregated with backup and copy job findings in the summary task.
...
Collect AWS Backup copy jobs created in the last 30 days in AWS account 025825898506 and region us-east-1, using the default review window from 30 days before assessment time through assessment time, and identify any jobs with failed, expired, aborted, completed-with-issues, or other non-success statuses.
...
- Use cli_session_command_execute to run AWS CLI commands against account 025825898506 in region us-east-1.
- Because execution_preferences.useDefaultValuesWithoutConfirmation is true, use the provided default review window without prompting the user: review_window_start_timestamp = 30 days before assessment time and review_window_end_timestamp = assessment time.
- Use cli_session_command_execute to compute or derive the review window timestamps at execution time and list AWS Backup copy jobs in scope for that window.
- Use cli_session_command_execute to retrieve copy job details for each job returned in the window, including job ID, status, timestamps, source and destination vault details, destination region/account details, recovery point identifiers, and diagnostic fields.
- Use cli_session_command_execute to filter copy jobs to non-success statuses and record all required fields, explicitly marking fields that do not apply or are not returned.
- Prepare the copy-job review output so it can be aggregated with backup and restore job findings in the summary task.
...
- Use the outputs from the three prior assessment tasks; no additional user confirmation is needed because execution_preferences.useDefaultValuesWithoutConfirmation is true for the provided review window.
- Aggregate the backup, restore, and copy job findings into combined reviewed-job counts and combined non-success-job counts for AWS account 025825898506 in region us-east-1.
- Produce a consolidated table of non-success jobs with all required identifying, source/destination, target, diagnostic, and timestamp fields, explicitly marking fields that are not applicable for a given job type.
- Summarize whether any non-success jobs were found and highlight any recurring failure patterns or, if none were found, state that all reviewed jobs were successful or had no non-success statuses in scope.
