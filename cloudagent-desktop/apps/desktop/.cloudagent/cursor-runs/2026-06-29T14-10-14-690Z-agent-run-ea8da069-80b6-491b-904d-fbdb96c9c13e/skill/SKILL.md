...
Collect AWS Backup backup, restore, and copy jobs created in the last 30 days in the target account and region, identify any non-success jobs, and preserve identifying and diagnostic details for each result.

**Completion Criteria**

- Used the target AWS account 025825898506 and region us-east-1 as the assessment scope
- Used the default review window start timestamp of 30 days before assessment time without prompting because useDefaultValuesWithoutConfirmation is true
- Used the default review window end timestamp of assessment time without prompting because useDefaultValuesWithoutConfirmation is true
- Listed AWS Backup backup jobs created within the last 30 days in the target account and region in scope
...
- Listed AWS Backup restore jobs created within the last 30 days in the target account and region in scope
...
- Listed AWS Backup copy jobs created within the last 30 days in the target account and region in scope
...
- Preserved enough identifying, source, destination, target, timestamp, and diagnostic detail for each non-success job to support final aggregation across backup, restore, and copy job failures
...
- Use cli_session_command_execute to confirm the active AWS caller identity and region context for account 025825898506 and region us-east-1 before collecting AWS Backup job data
- Use cli_session_command_execute to compute the review window using the provided default values without prompting: review_window_start_timestamp = 30 days before assessment time and review_window_end_timestamp = assessment time, because useDefaultValuesWithoutConfirmation is true
- Use cli_session_command_execute with AWS CLI in us-east-1 to list AWS Backup backup jobs created within the computed 30-day review window
- Use cli_session_command_execute with AWS CLI in us-east-1 to retrieve detailed metadata for backup jobs as needed so each job includes job ID, status, timestamps, protected resource identifiers, vault details if returned, and failure or status-reason fields
- Use cli_session_command_execute to filter backup jobs to all non-success statuses, including failed, expired, aborted, completed-with-issues, or any status outside the expected success state, and explicitly mark non-applicable fields for backup jobs
- Use cli_session_command_execute with AWS CLI in us-east-1 to list AWS Backup restore jobs created within the computed 30-day review window
- Use cli_session_command_execute with AWS CLI in us-east-1 to retrieve detailed metadata for restore jobs as needed so each job includes job ID, status, timestamps, source recovery point or resource identifiers, destination metadata if returned, restore target details, and failure or status-reason fields
- Use cli_session_command_execute to filter restore jobs to all non-success statuses, including failed, expired, aborted, completed-with-issues, or any status outside the expected success state, and explicitly mark fields that are not applicable or not returned
- Use cli_session_command_execute with AWS CLI in us-east-1 to list AWS Backup copy jobs created within the computed 30-day review window
- Use cli_session_command_execute with AWS CLI in us-east-1 to retrieve detailed metadata for copy jobs as needed so each job includes job ID, status, timestamps, source and destination vault metadata, destination region or account details if returned, and failure or status-reason fields
- Use cli_session_command_execute to filter copy jobs to all non-success statuses, including failed, expired, aborted, completed-with-issues, or any status outside the expected success state, and explicitly mark non-applicable fields for copy jobs
- Use cli_session_command_execute to normalize the collected backup, restore, and copy job results into a consistent structure that preserves job type, identifiers, timestamps, applicable source or destination fields, restore target details where applicable, and diagnostic messages for downstream summarization
...
Summarize the AWS Backup backup, restore, and copy job review results for the last 30 days and state whether any non-success jobs were found or all jobs were successful.
...
- Use cli_session_command_execute outputs from the assessment task to consolidate reviewed job counts and non-success job counts across backup, restore, and copy job types for the same 30-day window
- Use cli_session_command_execute to assemble a single combined result set of all non-success backup, restore, and copy jobs with normalized columns for identifiers, timestamps, resource details, applicable source or destination fields, restore target details, and failure diagnostics
- Produce the final human-readable summary and non-success jobs table based on the consolidated assessment results, explicitly noting fields that are not applicable and stating clearly if no non-success jobs were found
