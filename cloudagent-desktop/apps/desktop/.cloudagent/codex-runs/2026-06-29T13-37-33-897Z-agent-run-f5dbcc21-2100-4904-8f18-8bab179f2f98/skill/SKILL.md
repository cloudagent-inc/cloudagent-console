...
Collect AWS Backup backup jobs created in the last 30 days in us-east-1 for AWS account 025825898506 and identify any jobs with failed, expired, aborted, completed-with-issues, or other non-success statuses.
...
- Confirmed whether any non-success backup jobs were found during the 30-day review window from the default start timestamp of 30 days before assessment time to the default end timestamp of assessment time.
...
- Use cli_session_command_execute to verify the authenticated AWS CLI context is scoped to AWS account 025825898506 and region us-east-1 for this read-only assessment.
- Use cli_session_command_execute to list AWS Backup backup jobs in us-east-1 created between the default review_window_start_timestamp of 30 days before assessment time and the default review_window_end_timestamp of assessment time without prompting the user, because execution_preferences.useDefaultValuesWithoutConfirmation is true.
- Use cli_session_command_execute to retrieve detailed fields for each backup job returned in scope, including job ID, state/status, resource identifiers, vault details if present, timestamps, and diagnostic message fields.
- Use cli_session_command_execute to filter the collected backup jobs to non-success statuses such as FAILED, EXPIRED, ABORTED, COMPLETED_WITH_ISSUES, or any other status outside the expected success state.
- Record for each non-success backup job the required aggregation fields, explicitly marking destination vault, destination region, destination account, and restore target details as not applicable when AWS Backup does not return them for backup jobs.
- Produce a workload-independent assessment result for backup jobs in the live environment because delivery.target is direct_cloud and no workload-scoped tracked resources apply.
...
Collect AWS Backup restore jobs created in the last 30 days in us-east-1 for AWS account 025825898506 and identify any jobs with failed, expired, aborted, completed-with-issues, or other non-success statuses.
...
- Confirmed whether any non-success restore jobs were found during the 30-day review window from the default start timestamp of 30 days before assessment time to the default end timestamp of assessment time.
...
- Use cli_session_command_execute to list AWS Backup restore jobs in us-east-1 created between the default review_window_start_timestamp of 30 days before assessment time and the default review_window_end_timestamp of assessment time without prompting the user, because execution_preferences.useDefaultValuesWithoutConfirmation is true.
- Use cli_session_command_execute to retrieve detailed fields for each restore job returned in scope, including job ID, status, source recovery point ARN, protected resource details, timestamps, restore metadata, target identifiers, and diagnostic fields.
- Use cli_session_command_execute to filter the collected restore jobs to non-success statuses such as FAILED, EXPIRED, ABORTED, COMPLETED_WITH_ISSUES, or any other status outside the expected success state.
- Record for each non-success restore job the required aggregation fields, explicitly marking destination vault, destination region, destination account, or other target fields as not applicable or not returned when AWS Backup does not provide them.
- Produce a workload-independent assessment result for restore jobs in the live environment because delivery.target is direct_cloud and no workload-scoped tracked resources apply.
...
Collect AWS Backup copy jobs created in the last 30 days in us-east-1 for AWS account 025825898506 and identify any jobs with failed, expired, aborted, completed-with-issues, or other non-success statuses.
...
- Confirmed whether any non-success copy jobs were found during the 30-day review window from the default start timestamp of 30 days before assessment time to the default end timestamp of assessment time.
...
- Use cli_session_command_execute to list AWS Backup copy jobs in us-east-1 created between the default review_window_start_timestamp of 30 days before assessment time and the default review_window_end_timestamp of assessment time without prompting the user, because execution_preferences.useDefaultValuesWithoutConfirmation is true.
- Use cli_session_command_execute to retrieve detailed fields for each copy job returned in scope, including job ID, status, source recovery point ARN, source and destination vault details, destination region/account details if present, timestamps, and diagnostic fields.
- Use cli_session_command_execute to filter the collected copy jobs to non-success statuses such as FAILED, EXPIRED, ABORTED, COMPLETED_WITH_ISSUES, or any other status outside the expected success state.
- Record for each non-success copy job the required aggregation fields, explicitly marking destination account or restore target details as not applicable or not returned when AWS Backup does not provide them.
- Produce a workload-independent assessment result for copy jobs in the live environment because delivery.target is direct_cloud and no workload-scoped tracked resources apply.
...
- Validated that the summary period matches the assessment tasks and is stated as the last 30 days using the default review window from 30 days before assessment time to assessment time.
...
- Use the outputs from the three prior cli_session_command_execute assessment tasks as the sole input set for the summary, without performing additional environment-wide discovery.
- Combine backup, restore, and copy job findings for AWS account 025825898506 in us-east-1 across the default review window from 30 days before assessment time to assessment time.
- Produce a consolidated table of all non-success jobs with the required identifiers, source/destination fields, target details where applicable, timestamps, and diagnostic messages.
- State clearly whether any non-success jobs were found and, if none were found, state that all reviewed jobs were successful or had no non-success statuses in scope.
- Summarize recurring failure reasons or patterns only when supported by the collected assessment evidence.
