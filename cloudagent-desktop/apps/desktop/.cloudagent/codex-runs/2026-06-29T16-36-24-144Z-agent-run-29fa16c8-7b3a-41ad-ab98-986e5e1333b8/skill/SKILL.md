...
- Use cli_session_command_execute to run AWS CLI commands in the authenticated Development profile context for account 025825898506 and region us-east-1.
- Use the provided default values without confirmation because execution_preferences.useDefaultValuesWithoutConfirmation is true: set the review window start to 30 days before assessment time and the review window end to assessment time.
- Use cli_session_command_execute to calculate or format the review window timestamps as needed for AWS Backup list-backup-jobs filtering.
- Use cli_session_command_execute to call aws backup list-backup-jobs in us-east-1 and collect backup jobs created within the review window.
- Use cli_session_command_execute to inspect returned backup job details and isolate all jobs whose status is not a success state.
- Use cli_session_command_execute to capture for each non-success backup job the backup job ID, protected resource ARN or identifier, source vault if returned, failure/status message, and relevant timestamps.
- Use cli_session_command_execute to explicitly note fields that do not apply to backup jobs, including destination vault, destination Region, destination account, and restore target details when they are not returned or not applicable.
- Prepare structured assessment output for downstream aggregation with restore and copy job findings.
...
- Use cli_session_command_execute to run AWS CLI commands in the authenticated Development profile context for account 025825898506 and region us-east-1.
- Use the provided default values without confirmation because execution_preferences.useDefaultValuesWithoutConfirmation is true: set the review window start to 30 days before assessment time and the review window end to assessment time.
- Use cli_session_command_execute to calculate or format the review window timestamps as needed for AWS Backup list-restore-jobs filtering.
- Use cli_session_command_execute to call aws backup list-restore-jobs in us-east-1 and collect restore jobs created within the review window.
- Use cli_session_command_execute to inspect returned restore job details and isolate all jobs whose status is not a success state.
- Use cli_session_command_execute to capture for each non-success restore job the restore job ID, source recovery point ARN, protected resource ARN or identifier, source vault if returned, restore target metadata, failure/status message, destination Region/account details if returned, and relevant timestamps.
- Use cli_session_command_execute to explicitly note fields that are not applicable or not returned for each restore job, including destination backup vault where absent.
- Prepare structured assessment output for downstream aggregation with backup and copy job findings.
...
- Use cli_session_command_execute to run AWS CLI commands in the authenticated Development profile context for account 025825898506 and region us-east-1.
- Use the provided default values without confirmation because execution_preferences.useDefaultValuesWithoutConfirmation is true: set the review window start to 30 days before assessment time and the review window end to assessment time.
- Use cli_session_command_execute to calculate or format the review window timestamps as needed for AWS Backup list-copy-jobs filtering.
- Use cli_session_command_execute to call aws backup list-copy-jobs in us-east-1 and collect copy jobs created within the review window.
- Use cli_session_command_execute to inspect returned copy job details and isolate all jobs whose status is not a success state.
- Use cli_session_command_execute to capture for each non-success copy job the copy job ID, source recovery point ARN, protected resource ARN or identifier, source vault, destination vault, destination Region, destination account if returned, failure/status message, and relevant timestamps.
- Use cli_session_command_execute to explicitly note that restore target details do not apply to copy jobs unless AWS Backup returns related fields requiring documentation.
- Prepare structured assessment output for downstream aggregation with backup and restore job findings.
...
- Collect the structured outputs produced by the three prior assessment tasks.
- Use cli_session_command_execute only if needed to re-read or validate AWS Backup job details that are missing from the assessment outputs; otherwise summarize from the collected task outputs without additional environment mutation.
- Aggregate reviewed job counts and non-success job counts across backup, restore, and copy job types for the same 30-day window.
- Produce a combined table of all non-success jobs with job type, job ID, resource identifier, source/destination details, restore target details where applicable, diagnostic messages, and timestamps.
- Explicitly mark fields as not applicable when they do not apply to a given job type.
- State clearly whether any non-success jobs were found and summarize recurring failure causes if present.
