...
Collect AWS Backup backup jobs created in the last 30 days in AWS account 025825898506 and region us-east-1, using the default review window start timestamp of 30 days before assessment time and end timestamp of assessment time, and identify any jobs with failed, expired, aborted, completed-with-issues, or other non-success statuses.
...
- Use the default review window values without prompting because execution_preferences.useDefaultValuesWithoutConfirmation is true: review_window_start_timestamp = 30 days before assessment time, review_window_end_timestamp = assessment time.
- Use cli_session_command_execute to call AWS Backup list-backup-jobs for jobs in scope and paginate as needed for the last 30 days.
- Use cli_session_command_execute to retrieve detailed fields for each backup job returned in scope, including job ID, status, timestamps, resource identifiers, vault information, and diagnostic/status message fields available from AWS Backup.
- Filter the collected backup jobs to non-success statuses, including failed, expired, aborted, completed-with-issues, or any status outside the expected success state.
- Normalize each non-success backup job into a structured result that includes job type, job ID, protected resource identifier, source vault, destination vault applicability, destination region applicability, destination account applicability, restore target applicability, timestamps, and failure diagnostics.
- Record explicitly when destination vault, destination region, destination account, or restore target details do not apply to backup jobs.
- Document whether any non-success backup jobs were found in the 30-day window and retain the full reviewed count for the final summary task.
...
Collect AWS Backup restore jobs created in the last 30 days in AWS account 025825898506 and region us-east-1, using the default review window start timestamp of 30 days before assessment time and end timestamp of assessment time, and identify any jobs with failed, expired, aborted, completed-with-issues, or other non-success statuses.
...
- Use cli_session_command_execute to run AWS CLI commands in the authenticated session for account 025825898506 and region us-east-1.
- Use the default review window values without prompting because execution_preferences.useDefaultValuesWithoutConfirmation is true: review_window_start_timestamp = 30 days before assessment time, review_window_end_timestamp = assessment time.
- Use cli_session_command_execute to call AWS Backup list-restore-jobs for jobs in scope and paginate as needed for the last 30 days.
- Use cli_session_command_execute to retrieve detailed fields for each restore job returned in scope, including job ID, status, timestamps, source recovery point or resource identifiers, restore metadata, target details, and diagnostic/status message fields available from AWS Backup.
- Filter the collected restore jobs to non-success statuses, including failed, expired, aborted, completed-with-issues, or any status outside the expected success state.
- Normalize each non-success restore job into a structured result that includes job type, job ID, source identifiers, source vault, destination vault applicability, destination region/account details when available, restore target details, timestamps, and failure diagnostics.
- Record explicitly when destination vault, destination region, or destination account details are not applicable or not returned for a restore job.
- Document whether any non-success restore jobs were found in the 30-day window and retain the full reviewed count for the final summary task.
...
Collect AWS Backup copy jobs created in the last 30 days in AWS account 025825898506 and region us-east-1, using the default review window start timestamp of 30 days before assessment time and end timestamp of assessment time, and identify any jobs with failed, expired, aborted, completed-with-issues, or other non-success statuses.
...
- Use cli_session_command_execute to run AWS CLI commands in the authenticated session for account 025825898506 and region us-east-1.
- Use the default review window values without prompting because execution_preferences.useDefaultValuesWithoutConfirmation is true: review_window_start_timestamp = 30 days before assessment time, review_window_end_timestamp = assessment time.
- Use cli_session_command_execute to call AWS Backup list-copy-jobs for jobs in scope and paginate as needed for the last 30 days.
- Use cli_session_command_execute to retrieve detailed fields for each copy job returned in scope, including job ID, status, timestamps, source recovery point or protected resource identifiers, source vault, destination vault, destination region/account fields, and diagnostic/status message fields available from AWS Backup.
- Filter the collected copy jobs to non-success statuses, including failed, expired, aborted, completed-with-issues, or any status outside the expected success state.
- Normalize each non-success copy job into a structured result that includes job type, job ID, source identifiers, source vault, destination vault, destination region, destination account when available, restore target applicability, timestamps, and failure diagnostics.
- Record explicitly when destination account is not returned and when restore target details do not apply to copy jobs.
- Document whether any non-success copy jobs were found in the 30-day window and retain the full reviewed count for the final summary task.
...
- Use the outputs from the three prior assessment tasks as inputs; no additional user prompting is needed because the default review window has already been applied.
- Consolidate reviewed job counts and non-success job counts across backup, restore, and copy job types for AWS account 025825898506 in region us-east-1.
- Aggregate all non-success jobs into a single normalized table with fields for job type, job ID, resource identifier, source vault, destination vault, destination region, destination account, restore target details, timestamps, and failure diagnostics.
- Mark fields as not applicable where the job type does not support that field, rather than leaving blanks ambiguous.
- Summarize recurring failure messages or patterns if any non-success jobs are present.
- If no non-success jobs are present across all three assessments, state clearly that all reviewed jobs were successful or had no non-success statuses in scope.
- Produce a final internally consistent summary covering the last 30 days review window.
